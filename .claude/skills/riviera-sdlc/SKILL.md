...
| **Review** | **Mandatory gate.** **Always start `/code-review` — the overlay alone is NOT the review** — via the **invocation ladder** in `references/pr-gates.md` §1 (which rung to use, why the Skill tool refuses the name, and the degraded `/review <PR>` fallback all live there). A rejected invocation name is easy to misread as the gate being unavailable — a whole review was once run on the degraded fallback over exactly that; misreading it leaves the generic banks unrun. If tooling genuinely blocks every rung, say so in the PR and leave the box unticked rather than substituting silently. Review the PR diff against the invariants; record findings; each fix re-enters at Implement (re-entry rule). Green CI is not a substitute. | `riviera-review-overlay` + `/code-review` |
