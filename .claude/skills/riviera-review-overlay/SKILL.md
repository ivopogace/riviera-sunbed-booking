...
never runs alone: load when a review is active (`/code-review` or another engine) and the
work is in this repo; on an explicit user invoke, start the review first (the invocation
ladder in `riviera-sdlc` `references/pr-gates.md` §1). `/security-review` doesn't auto-load
this overlay; consult the reference files directly.
...
- **Backend diff** → `references/backend-conventions.md` (RV-BE-1..19: JDBC-only, Modulith
  ...
  placement, package shape, rule-layer placement, Flyway, session lifecycle). If the diff
  changes any wire shape (an endpoint, a DTO, an error body) — even with no frontend file
  touched — also load `references/fe-be-contract.md`.
...
- **Substrate diff, or a change to a structural test** → no reference file; RV-PROC-2 below
  is the item. "Only style items applied" is the failure mode it prevents.
...
- Prose that narrates the diff, restates the code, or records history the reader never saw —
  **Minor**; the guard cannot see this half, judge it by the test.
...
Not covered: `#` files (shell, YAML, `.properties`) and SQL `--` — judge by eye and lean
toward leaving them alone — and a one-line comment that shouldn't exist at all (default to
zero inline comments in a method). Don't reflow untouched comments.
...
`platform/`, and `frontend/`'s root files, listed in `.prettierignore`), formatting is a
matter of matching the surrounding file; judge by eye, lean toward leaving it alone.
...
`docs/agents/**` — or adds/tightens a fitness function (a test that fails the build on a rule
over production code the diff need not touch: an ArchUnit rule, Modulith's `verify()`, a
classpath probe, an endpoint census), which falsifies substrate examples from the other
direction. Such a diff otherwise draws only RV-STYLE-1/2, and those read the prose, never
whether it is still true. Three checks, re-walked on every re-review:

**a. Every citation still resolves.** Skills cite the tree by name and line
(`BookingStatus.java`, `JdbcAvailabilityClaim`, `V4__availability.sql:32`). Open every path,
line range, class and method named on a changed line, plus every citation of anything this
diff renamed, moved or deleted. **Major** when the diff's own rename broke the pointer,
**Minor** for one it merely sat beside.

**b. Every worked example still holds.** An example is code a session will copy, so it is
held to the structural net (membership rule: `riviera-modulith` § *The structural net*;
members: `CLAUDE.md` §Commands). Walk it both ways: every example on a changed line against
the net as it stands; and when the diff adds or tightens a net member, **every** example in
the substrate, **including files the diff never opened**. Tightening a target-naming rule
(`ErrorContractArchitectureTests`, `ResponsibilitiesArchitectureTests`, an
`*AuthPlacementTests`, `RetiredSetExclusionArchitectureTests`) puts a narrower walk due: grep
the substrate for the names read out of the test and walk those hits alone. A diff that adds
any other kind of rule (a review item, a skill's prose rule, a guard script) gets the first
walk alone. Sweep for the tightened test's own forbidden set, read out of the test rather
than remembered, over the substrate path set and no wider (a research note under `docs/`
records what was true when written, not an instruction a session copies):
...
A **command** is a worked example too. The unit is the pair — the command, and the prose
claiming what it covers — and touching **either** side puts the check due (widening a map
without widening the grep beneath it is the commoner half). Run the command, and match what
it actually reaches against every coverage claim in the document, including claims on lines
the diff never touched. **Major** — a procedure that cannot reach its own stated scope reports
zero findings, which reads as good news.

**c. `riviera-docs-freshness` step 2a over what the diff removed or renamed.** A retired
name is one the diff's `-` lines carry and the tree after the diff does not answer to — a
class, file, test, command, setting, label or item name, or a stated count — whether deleted
or replaced. Wording the diff generalized, reworded, or set a sibling beside retired nothing:
this check is N/A. Hand each retired name to the skill's rename/removal grep, then judge every
hit against the tree: a hit is a finding (**Major**) only when the line states, as present
fact, a name, path, count or mechanism the tree does not bear out. A hit the diff left true
is not a finding, and neither is one in historical narrative (an ADR's Context paragraph, a
research note). When the diff lands or re-decides an ADR, add the skill's step 3 over the
docs that stated the old position — a retired promise matches no identifier.
