...
- **Don't hand-flag `printWidth`, quote style, or wrapping** in `frontend/`. If the line is really
  ...
  failed the PR and named it. If that step is green, the line is fine, however long it looks — the
  raised-and-fixed round trip on PR #520 and the raised-and-rejected one on PR #612 are the two the
  guard exists to retire.
