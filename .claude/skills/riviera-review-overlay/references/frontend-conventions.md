...
- [ ] **migrate-on-touch:** a component the diff touches that still carries legacy component SCSS had that styling migrated to Tailwind in this same slice — or the SCSS is a justified holdout (scrim-class) / the migration is deferred with its stated why (`riviera-tailwind` owns the rule + checklist)
