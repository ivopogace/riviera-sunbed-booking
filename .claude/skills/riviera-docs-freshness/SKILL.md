...
| `docs/design/*` — `colour-literal-token-audit.md` and `non-text-contrast.md`, the folder's only two files | ledger rows still marked open for a family that shipped; a rule's family table citing a spec that does not measure what it claims | both are maintained — correct them in place. There is no drawn artboard to reconcile against any more; the executable design spec is the contrast/token-drift specs beside each component |
