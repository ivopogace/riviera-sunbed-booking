 ...
 *
 * <p>Shared by the cancel and controller ITs rather than copied into each: this is the most
 * delicate fixture in the fence's coverage, and two copies would drift.
 *
 * <p><strong>Re-run safe on a shared container.</strong> A backdated booking is deliberately never
 * released — that is what the fence asserts — so its {@code set_availability} row survives the
 * test. A second run would then move another row onto the same {@code (set_id, booking_date)} and
 * hit the unique constraint that enforces invariant #2, so any earlier residue at the target date
 * is cleared first.
 *
 * <p>{@code created_at} moves with {@code booking_date}: a real advance booking is created before
 * the date it's for, so an honest fixture backdates both. The service-day row the schema wrote at
 * confirm moves too, or the sweep would still see a service day ahead. The mechanism now also
 * serves the abandoned sweep's day-end arm ({@link
 * ai.riviera.platform.booking.application.BookingCutoff#lastEndedServiceDay}).
 */
final class ServiceDayBackdate {
	...
	void moveToPast(String code, LocalDate past) {
		...
		jdbc.sql("""
				UPDATE booking_day SET service_date = :past
				WHERE booking_id = (SELECT id FROM booking WHERE code = :c)
				  AND service_date = (SELECT booking_date FROM booking WHERE code = :c)
				""")
