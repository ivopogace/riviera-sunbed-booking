...
/**
 * HTTP contract for self-service erasure ({@code POST /api/me/erasure}, [D5]) through the real filter
 ...
 * {@code /api/me/**} matcher, which replaced this endpoint's dedicated one; that it still holds
 ...
 * <li><strong>The revoke brackets the scrub</strong> (AC-11): it runs before <em>and</em> after, and a
...
class MeErasureControllerTest {
	...
	/**
	 * The revoke must run <strong>before</strong> the scrub. Ordered the other way (as this shipped),
