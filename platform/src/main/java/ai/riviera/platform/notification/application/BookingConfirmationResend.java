...
/**
 * Send one booking's confirmation mail again, on request (#380) — the driving port behind the admin
 ...
 * ({@code riviera.notification.mail.socket-timeout-ms}, #410), so doing it on the caller's thread
 ...
 * Event Publication Registry's idempotency for the automatic send (#371) — the whole point is to send
