...
/**
 * A venue declined a pending Request-to-Book (issue #124): the guarded
 ...
 * resolver does not return it (#373's precedent). Never the booking code — the registry persists
 * payloads in cleartext (invariant #7).
 *
 * <p>The withdraw leg beside decline publishes <strong>no</strong> counterpart, deliberately
 * (#123): the guest retracted the request themselves, so there is no outcome to notify them of.
 */
