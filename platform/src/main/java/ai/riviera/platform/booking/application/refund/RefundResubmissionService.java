...
/**
 * The ADMIN refund-resubmit lever: this module's scope ({@link RefundOutbox}, the refund-bulkhead
 * listeners), window ({@link RefundResubmissionWindow}) and log noun behind the kernel's
 * {@link ResubmissionThrottle}, which owns what a press means (RESPONSIBILITIES.md §shared).
 * A duplicate <em>refund</em> is prevented below the throttle: the gateway call is keyed
 * {@code booking-<id>-refund}; the registry's {@code markResubmitted} claim skips in-flight ones.
 * Never logs a booking id or code (invariant #7): it sees publication counts, never payloads.
 */
@Service
