 ...
 * (invariant #6). Rating/reviews are not operator input — a new venue starts at zero, and the
 * commission rate is deliberately absent: it is the platform's term, stamped by the application
 * service from {@link VenueCreationProperties}, so no driving adapter can supply one. The edge
...
public record NewVenueCommand(String name, String beach, String region, String description,
		String bookingMode, String payoutCurrency, LocalTime bookingCutoff) {
