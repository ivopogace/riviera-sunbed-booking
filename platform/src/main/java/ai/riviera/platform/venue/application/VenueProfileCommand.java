 ...
 * {@code Europe/Tirane} wall-clock {@code LocalTime}, invariant #4/#6) — plus the amenity set (an
 * order-insensitive subset of the fixed {@link Amenity} catalogue) and the optional
 * distance-to-water in metres ({@code null} = not stated).
 *
 ...
 * to {@code Amenity}); the remaining edge invariants are checked in the canonical constructor via
 * {@link VenueFieldValidation}, shared with {@link NewVenueCommand}. The amenity set is defensively
 * copied so it is immutable and order-insensitive.
 */
public record VenueProfileCommand(String name, String beach, String region, String description,
		String bookingMode, LocalTime bookingCutoff, Set<Amenity> amenities, Integer distanceToWaterM) {
