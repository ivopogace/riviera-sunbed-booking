...
public enum SalesClose {
	...
	/**
	 * The one {@code HH:mm} wire form for venue wall-clock settings: every read formats through it
	 * so served surfaces cannot drift (the FE keys copy on exact string equality). Writes parse ISO
	 * input and revalidate via {@link #fromTime}.
	 */
