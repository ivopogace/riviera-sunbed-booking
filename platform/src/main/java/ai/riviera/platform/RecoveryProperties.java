...
/**
 * Tunables for account recovery. Bound from {@code riviera.recovery.*} (defaults documented in
 ...
 * served same-origin by the backend, so in demo/prod it is that origin — set
 * {@code RIVIERA_RECOVERY_LINK_BASE_URL}; local dev is the separate {@code :4200} SPA dev server).
