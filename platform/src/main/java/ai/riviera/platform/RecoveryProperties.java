 ...
 * served same-origin by the backend since #110, so in demo/prod it is that origin; local dev is the
 * separate {@code :4200} SPA dev server). It is cosmetic until the real mailer ships — the mock records
 * the link and the mocked e2e stubs the API.
 *
