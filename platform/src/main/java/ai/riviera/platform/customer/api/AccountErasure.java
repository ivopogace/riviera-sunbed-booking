 ...
 * child rows are deleted. The retained booking / payment / payout financial rows are <strong>never</strong>
 * touched — they are kept under the statutory-retention exception, which is exactly why the payout ledger's
 * auditability (invariant #9) is preserved (it holds no PII). Both operations are idempotent (guarded on
 * {@code erased_at IS NULL}); the edge authenticates the caller and revokes the erased subject's sessions.
 */
