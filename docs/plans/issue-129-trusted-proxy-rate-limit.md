...
- [x] **AC-7 (manual, merge close-out): PASSES on its literal wording, but the probe exposed a
  ...
  **Closed by #286** (2026-07-22): the config stopgap restored one-bucket keying in production
  (re-measured: 11 allowed / 189 throttled of 200), and #286 then removed the dependency on
  that hand-copied Cloudflare CIDR list — behind a trusted peer the resolver prefers a
  configurable edge-supplied client-IP header (shipped default `CF-Connecting-IP`), keeping
  this walk as the fallback. See `docs/plans/issue-286-durable-client-ip.md` and
