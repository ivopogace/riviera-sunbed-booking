...
| F-16 | docs verification pass (advisory) | `VenueTab` resets its "Saved" notice with an `effect` that reads signals only to write another — the shape Angular's docs say to replace with `linkedSignal`. The per-draft split is what let #1099's new draft miss the reset in the first place | deferred → issue #1119. The effect predates this slice, which extended it by one tracked signal; refactoring it here would widen the PR past the finding |
