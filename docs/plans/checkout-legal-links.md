...
| F-3 | review (RV-FE-3, score 75) | Terms promised "the exact closing time is shown while you book" — the tourist UI shows only a display default ("6 PM"), the per-venue cutoff is never rendered; TSDoc made the same claim | fixed in review-fix commit <!-- as-built: #804 later ships the per-venue sales-close rule on the venue map --> |
...
| F-9 | review (glass-tokens TSDoc, score 70) | New contrast spec hand-copied card-glass constants against the "ONE test-side mirror" rule | fixed: constants promoted into `glass-tokens.ts`, spec imports them (pre-existing copies deferred → issue #465; swept via PR #467 — 17 specs, not 4) |
