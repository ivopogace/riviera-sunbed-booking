...
- `riviera-docs-freshness` — run over `origin/main...HEAD` (pre-merge smoke, close-out step 5) after review
  finding F-6. Confirmed no substrate doc still names the retired `ApprovalOutcome` enum constants, and
  surfaced two ADR-0011 staleness items (F-7 patched, F-8 flagged to the maintainer).
- `riviera-review-overlay` — loaded at the review gate; its backend bank walked against the diff,
  producing finding F-1 (RV-STYLE-1). The full `/code-review` fan-out then ran (F-2 resolved) and found
  F-3..F-6. RV-PROC-1 re-checked after the fix round: it touched backend Java **and** substrate docs —
  the latter is why `riviera-docs-freshness` now appears above, which was itself finding F-6.
...
| F-4 | review (agent 4 — prior-PR comments) | **A recurring drift class, flagged on #427, #430 and #436 before this.** Adding a `kind` to the shared dispatcher left sibling enumerations stale: `TransactionalMailService.KIND_TAG` still said "two audiences" twelve lines above the new third kind, and `ObservabilityMetrics.MAIL_RECOVERY_DROPPED`/`MAIL_RECOVERY_FAILED` still described themselves as carrying "recovery mails (email verification, password reset)" — contradicting `docs/runbooks/observability.md` *in this same PR* | fixed — all three Javadocs now state the vehicle-vs-flow reading and name the third kind. This is the same fix #430 applied to `MAIL_REGISTRY_SHED`, now applied to its two siblings |
