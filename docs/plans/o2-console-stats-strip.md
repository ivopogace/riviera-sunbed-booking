...
| 2026-07-07 | CI red (full-suite-only): 15 context-load failures | sliced/isolated contexts needing the new controller/service ports | CI run 684 failure list → `@WebMvcTest`+`WebSliceStubs`, payout `@ApplicationModuleTest` | `WebSliceStubs` (11 RateLimit* + 2 WebCorsConfig), `PayoutModuleTest` (1) | Stubbed `ViewDailyTakings` in `WebSliceStubs` and mocked `booking.api.DailyTakings` in `PayoutModuleTest`. **Lesson:** a new `@RestController` or a new cross-module port dependency must be added to the web-slice stubs + every module-isolation test's mocks — invisible to scoped runs, only the full CI suite shows it (riviera-local-debug full-suite-only class) |
