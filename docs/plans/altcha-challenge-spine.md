...
seam, one behavior per cycle) · `riviera-review-overlay` (review gate — **ran** at ready-for-review on PR #911 via `code-review:code-review` at high effort: five generic review agents plus the overlay bank walk; findings F-5..F-10 below, all fixed or decided) ·
`riviera-docs-freshness` (**ran** over `origin/main...HEAD` at phase 5 — the counting sweep for the *sixth* `@Scheduled` job, the *eleventh* rate-limit dimension map and the *fifth* bounded scheduled-query client found 3 stale statements, all patched: two "four" phrasings in `ScheduledQueryTimeout`'s Javadoc and `CLAUDE.md`'s Platform-edge summary, which now names the fence; the rename grep for the path helper moved into `RequestPaths` found nothing; `production-hardening.md`'s "two lockless sweeps" heading names the two that break at N>1 and stays true) · `grilling` (the intake interrogation above) · `riviera-local-debug` (loaded before the session's first build: the JDK 21 daemon + JDK 25 toolchain recipe, one-class-at-a-time ITs on the hook's `dockerd`, the full-suite-only failure class that shaped the ITs' unique IPs and the sweep's initial delay, and the container-local stack for the real-backend journey) · `domain-modeling` (loaded at the review-fix round — the glossary keeps *proof-of-work challenge* implementation-free and lists *bot check* / *human check* under *Avoid*, so the widget's status and rejection copy now says *security check*; no new glossary term, the entry from #908 already covers the slice) ·
...
| F-5 | review (agent #5, scored 75) | `RateLimitFilter`'s class Javadoc kept calling every non-booking endpoint a credential/mail oracle after the challenge GET joined the list | fixed: the sentence names the challenge GET's budget as a solve-cost cap |
| F-6 | review (agent #5, scored 75) | `challengePayload` survived a mode/audience toggle, so a remounted widget could start from the previous card's solution | fixed: the reset effect clears it; `auth-page.spec.ts` pins the remount |
| F-7 | review (overlay, RV-PROC-1 Major) | *Skills consulted* omitted `domain-modeling` (routing row for a new JDBC adapter + controller) and `riviera-local-debug` (local builds) | fixed: both loaded/recorded; `domain-modeling` re-vet changed the widget copy per the glossary's *Avoid* list |
| F-8 | review (overlay side note) | `JdbcChallengeRegistry` read the scheduled query timeout via a raw `@Value` where the root's other adapter injects the validated `ScheduledQueryTimeout` bean | fixed: injects the bean |
