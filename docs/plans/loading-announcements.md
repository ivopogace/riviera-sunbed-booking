...
- **Failure announcements.** Settled at round 10, after several rounds miscounted it in both
  directions. The house pattern is `role="alert"` on the failure panel; what matters is that the
  check is **per branch**, because an `alert` elsewhere in a file often belongs to a submit or
  delete flow rather than to the load. Read that way, this slice's eight surfaces fall in three
  groups. **Covered** — `home` (`error`), `venue-map` (`map-not-found`, `map-error`) and the
  `daily-view-tab` / `requests-tab` / `payouts-tab` error paragraphs (`daily-load-error`,
  `requests-load-error`, `payouts-load-error`), each an `alert` inserted on its own load-failure
  branch. **Covered from its parent** — `set-editor`, whose `loaded` is an
  `input.required` and whose only `alert` is a *save* error; its load failure is
  `layout-editor`'s `layout-load-failed` panel, so re-check it there, not here. **Silent — and the
  whole of the follow-up scope:** `my-bookings`' `booking-row-failed` (no role at all; round 4 gave
  it `role="alert"` and round 5 reverted that as a per-row assertive region, F-25) and its
  `account-error` (`role="status"`, but born holding its text inside its `@if`); plus
  `set-password`'s signed-out branch (`setpw-signed-out`), whose three `alert`s all belong to the
  password-submit and erase flows. That last one needs design, not an attribute: `session-auth`
  folds a failed restore into the same signed-out principal, so "Sign in to manage your account."
  is what a 500 renders too, and a panel has to exist before it can carry a role. Surfaces are
  named by `data-testid`, and no app-wide count is recorded, both on purpose (G-8).
...
| 12 — Seventh-round findings F-40…F-45 (round 6's inventory corrected) | ✅ | |
| 13 — Eighth-round findings F-46…F-48 (census removed from durable docs; scope corrected) | ✅ | |
| 14 — Ninth-round findings F-49…F-53 (docs and comments only; no behavioural change since round 6) | ✅ | |
| 15 — Tenth-round findings F-54…F-57 (duplication fixed at the root; review gate converged) | ✅ | |
...
| F-46 | **eighth review round** (re-review of the round-7 diff) | ⚠️ **Its "no count in either durable doc" claim was overstated — see F-52, and round 10, which removed the last confusing remnant.** **The census was wrong a third time, so it is gone rather than corrected again.** Round 7's "7 of 8 surfaces covered / 56 occurrences" counted `set-password` and `set-editor` as covered on the strength of `role="alert"` elements that belong to their *submit* and *erase* flows — `set-password`'s actual load non-success branch (`@else if (!auth.signedIn())`) has no role at all, and `set-editor` has no load-failure branch of its own. The quoted 56 does not reproduce from the command G-8 states (raw grep: 68) | fixed — **no count in either durable doc.** RV-FE-10 and `load-announcer.ts` now carry the rule plus the check ("read the branch, and read the *right* branch — an alert elsewhere in the file may be a submit flow's"); the dated per-surface finding stays in this plan. G-8 rewritten around the three-round arc |
| F-47 | eighth review round | flowing from F-46: `set-password`'s silent branch was missing from the follow-up scope, and the checklist said "do not widen it" — an instruction that would have told the next agent not to add it | fixed — it is in scope, with the note that it needs design rather than an attribute: `session-auth` folds a failed restore into the same signed-out principal, so there is no failure panel to carry a role yet. The "do not widen" instruction is replaced by "re-check rather than assume" |
| F-48 | eighth review round (noted, not reported) | three small doc errors in the round-7 text: `payouts-tab`'s branch is `@else if (loadErrorMsg(); as msg)`, not `loadError()`; the population was described as "two surfaces" (itself overtaken by F-47 the same round — with `set-password` it is three branches across two surfaces); and `load-announcer.ts` looked to have gained over-width comment lines | the first two fixed. **The width part was wrong and is withdrawn (F-49):** it was measured in bytes, so an em-dash or `…` counted twice; the two lines "fixed" were exactly 100 characters, and the file's three genuinely 101-char lines — present since it was created — were never touched |
| F-49 | **ninth review round** (re-review of the round-8 diff) | F-48's width claim was measured with `awk length`, which counts **bytes**: the two lines it rewrapped were exactly 100 characters (102 bytes, thanks to `…`), and the three lines actually over `printWidth: 100` — untouched, and present since the file was created — are still there. The register recorded a fix that fixed nothing and a comparison ("fewer than before this slice") against a file this slice created | fixed — F-48's row corrected and the claim withdrawn. The three 101-char lines are left alone deliberately: `format:check` passes (Prettier does not rewrap comment prose) and `my-bookings.ts` carries 21 of the same kind. Chasing one character is not worth a diff |
| F-50 | ninth review round | F-46 and G-8 both said *both* durable docs now carry the "read the **right** branch" check; only RV-FE-10 had it. The doc a developer actually reads at the call site — `load-announcer.ts`'s TSDoc — was missing the exact check whose absence made round 7's census wrong | fixed — the clause added there too, so the claim is now true |
| F-51 | ninth review round | the Non-goals bullet attributed `@else if (loadError())` to all five covered surfaces; `home` is `@if (failed())` and `venue-map` is `@else if (notFound())` / `@else if (failed())`. Three of five wrong, in the sentence announcing branch-by-branch rigour — and it pinned `home.html:131`, a bare line number, as its one citation | fixed — the bullet no longer asserts branch **syntax** at all, and names surfaces by `data-testid`, which does not drift when lines move |
| F-52 | ninth review round | RV-FE-10's new parenthetical claimed the item "deliberately states no inventory" while the same item still says #741 shipped the shape "on **eight** surfaces" — and F-46 claimed "no count in either durable doc" | fixed — the parenthetical now distinguishes the two: no *current* inventory of who carries a role (that decays), while the dated #741 history stays |
| F-53 | ninth review round (noted, not reported) | three register/checklist errors: F-40 and F-41 stood unmarked though F-46/F-47 overturn them (this table's convention is to annotate a reversal inline, as F-17 does); round 8's rejection was filed under round 7's heading; and the checklist collapsed `set-editor` into "covered" when the Non-goals bullet makes it a third category, while citing F-40/F-41/F-46 for an `appFailurePanel` point that only F-34 evidences | fixed — all three |
| F-54 | **tenth review round** (re-review of the round-9 diff) | **The root cause behind rounds 7–9, finally named:** the "read the right branch" rule existed in **five** near-identical copies across three files (RV-FE-10, `load-announcer.ts`'s TSDoc, the Non-goals bullet, G-8, F-46's outcome), each worded slightly differently and each an independent drift site. F-50 — a correction that reached one of two docs — *was* that bug, one round old. Round 9's fix added the fifth copy | fixed structurally — RV-FE-10 owns the rule; the TSDoc states this component's contract and points there for the call-site checklist (which also restores it to contract-only, the F-42 class). G-8 rewritten around the whole seven-round arc |
| F-55 | tenth review round | the three operator tabs were named by `data-testid`s that resolve to their **loaded** sections, not their alert paragraphs (`daily-load-error`, `requests-load-error`, `payouts-load-error`) — so the citation style F-51 adopted for verifiability was not verifiable for three of eight | fixed |
| F-56 | tenth review round | RV-FE-10's parenthetical quoted the "#741 shipped that shape on eight surfaces" note loosely, and sat at the end of the paragraph about `role="alert"` coverage — so a dated fact about the *born-with-text shape* read as a claim that eight surfaces carry a role, re-acquiring the census F-46 deleted. A third durable doc (the overlay's `SKILL.md`) carries the same note and had never been in scope | fixed — the parenthetical no longer quotes or gestures at that note; it states plainly that the item records no current inventory. The `SKILL.md` and RV-FE-10 notes are accurate history about the born-with-text shape and stay as they are |
| F-57 | tenth review round (noted, not reported) | register bookkeeping: F-27, F-28 and F-46 lacked the inline reversal marker F-53 had applied to F-40/F-41; G-8 had not been extended past round 8; F-48's "two cards on one surface" was overtaken by F-47 the same round; F-49 replaced an unchecked claim with another unchecked one (line provenance); the Non-goals bullet still said "settled at round 8" after round 9 rewrote it; and phase 14 said "all documentation" of a diff that edits a `frontend/src` file (comment-only) | fixed — all six |
...
**Considered and rejected in round 8:**

- *"`role="alert"` is 'the one live-region case with good support' undercuts this very component,
  whose own region is a polite `role="status"`."* — Fair as written, so the sentence was scoped
  rather than the claim rejected: it is the one case announced reliably **without a prior
  mutation**. Polite mutation announcements are exactly what this component relies on, and they
  are well supported; insertion-without-mutation is the case that is not.
...
| G-8 | Review rounds 6–10 | **"a claim asserted in a durable doc, never checked."** It ran seven rounds. Round 6 wrote an inventory of other components' markup into RV-FE-10 and `load-announcer.ts`, inherited from a finding. Round 7 corrected it and wrote a *new* count. Round 8 found that wrong too (two of its "covered" surfaces carry their `alert` on a **submit** branch) and deleted the census. Rounds 9–10 then found the *replacements* drifting: a check claimed for both docs that lived in one, three of five branch attributions wrong, three surfaces named by the wrong `data-testid` | every count and half the attributions written into a durable doc across five rounds were wrong, in both directions | **Fixed structurally in round 10, not restated a fifth time.** The root cause was duplication: the same rule had been copied into five places, so each correction reached some and missed others. RV-FE-10 is now its single owner; `load-announcer.ts`'s TSDoc states only this component's contract and *points* at RV-FE-10 for the call-site checklist. Neither carries a current inventory. The rule that falls out: a rule gets **one** home, and a doc that outlives its PR states rules, never censuses — dated, per-surface findings belong in the plan, where they are scoped |
...
- [x] Follow-up issue filed for the silent failure branches (Non-goals) — **#745**. `my-bookings`'
      ...
      checked branch-by-branch: five carry their own `alert`, and `set-editor`'s lives in its
