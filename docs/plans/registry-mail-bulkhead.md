...
| 7 — close-out `/code-review` fan-out over the close-out diff (F-6) | ✅ | one finding, fixed in the close-out commit |
...
| F-6 | **review gate, close-out fan-out** (reviewer 5, comment-accuracy pass) | The F-5 row below cited the corroborating evidence as "Phase 2's own step". It is **Phase 1's Step 2** (`→ FAIL (AC-1 times out: mail is on the shared executor; AC-7 fails: a transaction is active)`); Phase 2's Step 2 is about `MailListenerExecutorArchitectureTest` and reads only "→ FAIL; restore". A misattribution introduced by the very row that corrects a drifted claim, caught by checking the citation against the file rather than trusting it. Corrected here and in the PR body. The same reviewer re-derived the two load-bearing claims from vendored bytecode rather than prose — Boot 4.1's `TaskExecutionProperties$Pool` compiles in `coreSize=8` with `queueCapacity=Integer.MAX_VALUE` (so the pool never grows past 8), and `HikariConfig` defaults `maxPoolSize=10`, with no override anywhere in `src/main/resources` or `src/test` and no virtual-threads flag set — and confirmed `spring-tx` 7.0.8's empty-transaction branch plus `spring-jdbc`'s `DataSourceUtils.doGetConnection` bind path are exactly as F-4's Javadoc describes | fixed-in-this-commit |
...
- [x] **The review gate ran in full — both halves.** `riviera-review-overlay` layered onto `/review`
      ...
      **Re-run at close-out** over the F-4/F-5 diff, per the re-entry rule — the same five lenses,
      returning **one finding (F-6)**, fixed in this commit. Two reviewers independently reproduced the
      F-4 non-vacuity proof (`NOT_SUPPORTED` → the transaction assertion passes, the connection assertion
      fails) instead of taking this document's word for it.
