...
| **Tourist:** sea-banner gradient `#0e7a89→#0c6675`; card padding `px-4`; card elevation (`backdrop-blur-[26px]` + the `0_14px_44px` drop shadow, `rounded-[28px]`) | **preserved** | The unified frame adopts the tourist gradient AND (maintainer-caught, F-10) the tourist elevation — the first frame swap silently dropped the drop shadow, leaving the card edge invisible on the bare page gradient; restored on the shared frame (all consumers inherit it) and pinned by a computed `box-shadow` e2e check. Card padding becomes the frame's `px-[18px]` (cosmetic); mask/rest-offset coupling (16px ↔ inner `px-4`) viewport-internal and kept |
...
| F-10 | maintainer (visual check of the porcelain screenshots) | The frame swap silently dropped the tourist grid card's elevation (`backdrop-blur` + the `0_14px_44px` drop shadow, `rounded-[28px]`) — the card edge below the wash all but vanished on the bare page gradient; the parity ledger had mis-filed the swap as "cosmetic" | fixed in the elevation commit — the shared frame carries the restyle elevation for every consumer; pinned by a computed `box-shadow` check in `venue-map-pan.e2e.ts`; ledger row corrected |
