...
| 2026-08-21 | Phase 2 (post-edit) | Two mechanisms, because a surface can carry the hint without ever naming it: **(1)** everything that *reads* the hint — `scroll-hint` test id, its old and new sentences, the ✦ glyph; **(2)** everything that *mounts* the canvas with drag-pan on, derived from the mount sites rather than from memory of which surfaces "have maps" | `grep -rn "scroll-hint\|Drag or swipe\|bigger than your screen\|✦" frontend/src frontend/e2e` and `grep -rln "app-beach-map-canvas" frontend/src/app --include=*.html` piped through a `dragPan]="false"` test | **(1)** `beach-map-canvas.spec.ts` (12 gating reads + the 1 new copy read), `venue-map-pan.e2e.ts` (7 visibility reads) — no other copy assertion anywhere; **(2)** `venue-map.html`, `operator/daily-view-tab.html`, `operator/set-editor.html` adopt the line; `operator/layout-editor.html` is `dragPan=false` and never renders one | Shared-chrome change applied once, all three drag-pan surfaces re-run green (203 specs). The ✦ and its `&ngsp;` are gone repo-wide — the glyph had exactly one occurrence, which the mechanism sweep confirms rather than assumes |
