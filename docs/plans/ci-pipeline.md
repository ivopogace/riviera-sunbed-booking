...
**Goal:** Every PR into `main` — and every push to `main` itself — builds both apps, runs
their tests, and runs vulnerability + code-quality scans; a PR into `main` is only
mergeable when the required checks are green.

> **Amended by #417 (PR #418).** This originally read "every push (all branches)". An
> unscoped `push:` trigger made a PR branch fire **two** runs per SHA publishing the same
> check names, and branch protection honours only the last to finish — so a skipped
> duplicate could mask a real success and block a fully green PR. `push:` is now scoped to
> `main`, leaving PR branches to the `pull_request` event alone. A branch therefore has no
> CI until its PR exists, which is why `riviera-sdlc` opens the PR as a draft as soon as
> the first phase commit exists.
...
- ~~**Assumption:** the maintainer wants CodeQL + both CI jobs as the **required**
  ...
  `SonarCloud Code Analysis` already implies — so **#419** dropped it. That left **6**, verified
...
- `sonar-project.properties` (repo root) — **new (Phase 4, revised).** **Single**
  SonarCloud project (`org swap-shop`, key `ivopogace_riviera-sunbed-booking`) covering
...
> **Decision (revised after SonarCloud setup):** the maintainer imported the repo as a
> **single** SonarCloud project (`org swap-shop`, key `ivopogace_riviera-sunbed-booking`).
> Two separate scans pushing to one project key would overwrite each other, so the FE +
> BE coverage is fed to **one combined scan** in a dedicated `sonar` job (`needs:
> [backend, frontend]`): the build jobs upload the JaCoCo XML + compiled classes and the
> Vitest lcov as artifacts; the `sonar` job downloads both, normalizes the lcov `SF:`
> paths to repo-root (so coverage maps under the single project), and runs one
> `sonarqube-scan-action`. Still no Gradle Sonar plugin (avoids a fragile version pin).
> The whole `sonar` job is secret-gated — skipped/green until `SONAR_TOKEN` exists.
...
2. ~~Create the SonarCloud project.~~ ✅ **Done** — single project `ivopogace_riviera-sunbed-booking`
   under org `swap-shop`, Automatic Analysis disabled (CI-based). Keys wired into the
