...
- [ ] **AC-5:** Given `@vitest/coverage-v8` installed, when CI runs
  `ng test --coverage --coverage-reporters lcovonly`, then `frontend/coverage/lcov.info`
...
|-------|--------|---------|
| 0 — Backend coverage tooling (JaCoCo) | | |
| 1 — Frontend coverage tooling (Vitest v8) | | |
| 2 — Core CI workflow (backend + frontend jobs) | | |
| 3 — CodeQL + Dependabot | | |
| 4 — SonarCloud wiring (secret-gated) | | |
...
- `platform/build.gradle` — **modify.** Add `id 'jacoco'`; enable XML report; make the
  report run after `test`. (Sonar plugin added in Phase 4.)
...
- `frontend/sonar-project.properties` — **new (Phase 4).** FE Sonar project key/org +
  `sonar.javascript.lcov.reportPaths=coverage/lcov.info`, sources/exclusions.
...
- [ ] **Step 3 (green):** `npm run test:coverage` → tests pass and
  `frontend/coverage/lcov.info` exists. (Fully verifiable locally on Node 26 — no JDK.)
...
**Files:** Modify `platform/build.gradle`, `.github/workflows/ci.yml`; create `frontend/sonar-project.properties`

- [ ] **Step 1: Implement BE** — add `id 'org.sonarqube' version '<current 6.x>'` to
  `platform/build.gradle`; configure `sonar { properties { … projectKey, organization,
  host.url=https://sonarcloud.io, coverage.jacoco.xmlReportPaths=build/reports/jacoco/test/jacocoTestReport.xml } }`.
- [ ] **Step 2: Implement FE** — `frontend/sonar-project.properties` with
  `sonar.projectKey`, `sonar.organization`, `sonar.sources=src`,
  `sonar.javascript.lcov.reportPaths=coverage/lcov.info`, sensible `sonar.exclusions`.
- [ ] **Step 3: Gate in `ci.yml`** — set job-level `env: SONAR_TOKEN: ${{ secrets.SONAR_TOKEN }}`;
  add Sonar steps with `if: ${{ env.SONAR_TOKEN != '' }}` — BE runs `./gradlew sonar`,
  FE runs `SonarSource/sonarqube-scan-action@v4` (working dir `frontend`). Absent
  secret → steps skipped → green.
- [ ] **Step 4 (green):** YAML/gradle config parses; `./gradlew help` (JDK-21 smoke)
  resolves the sonarqube plugin. Live Sonar verification happens after the maintainer
  wires the secret (AC-7).
...
- [ ] **AC-5:** `frontend/coverage/lcov.info` produced. Verified locally + in CI artifact.
