...
(due at close-out — the console pin sentence in `riviera-frontend`, `admin-console-tabs.ts`'s
"rendered only inside AdminConsole" TSDoc, `e2e/support/shell.ts`'s two-prefix TSDoc, the
operator-console and admin artboards) · `grilling` (the intake questions answered from the code,
...
- [ ] **AC-1:** Given a signed-in admin operator, when `/operator/1/daily`, `/admin`,
...
- [ ] **AC-2:** Given the shell, when the section is `venue` with a venue id, then the venue slot
...
- [ ] **AC-3:** Given the shell at 390px on a page taller than the viewport, when the page is
...
- [ ] **AC-4:** Given the beach-map tab at 1280×900 with a 24-set, 12-column fixture, when the
...
- [ ] **AC-5:** Given a signed-out visitor on `/admin/audit`, when the page renders, then the
...
- [ ] **AC-6:** Given the tourist theme stored as `dark` and as `riviera`, when each of the four
...
- [ ] **AC-7:** Given a signed-in operator on each of the four routes, when `Sign out` is chosen
...
- [ ] **AC-8:** Given each of the four routes at 390px and at 1280px, when swept, then every
...
- [ ] **AC-9:** Given the merge, when `riviera-docs-freshness` runs over the range, then no
...
|---|---|---|---|---|---|---|
| R-1 | Two subscribers to `ConsoleVenueMap.load()` (the shell for the name, the console for the strip) fire two `GET /api/venues/1` and the console spec's `expectOne` goes red | med | low | the snapshot coalesces same-key reads; the console spec never mounts the shell (it is App's), the app spec flushes one read; verified by `httpMock.verify()` in both | agent | open |
| R-2 | A venue switch (`/operator/1/daily` → `/operator/2/daily`) leaves venue 1's name in the row (invariant #13) | low | high | the shell clears `venueName` and bumps a generation counter per `venueId`; `console-venue-switch.spec.ts` (real routes) asserts the row reads venue 2's name and nothing of venue 1 | agent | open |
...
| R-5 | Sticky + `translate-y` on the header changes the popover anchoring (chip and switcher popovers are `absolute` inside the header row) | low | med | the row stays `relative`; the popovers move with the header, which is the intended reading; e2e opens both at 390px after a scroll | agent | open |
...
| R-7 | Deleting `oc-footer` / the console `<main>` breaks e2e cases that locate them | med | low | `grep -rn "oc-footer\|oc-main\|locator('main"` (two hits: the console spec, `theme-shell.e2e.ts` reads App's main — unchanged) | agent | open |
...
- [ ] **Step 1: Write the failing tests** — a host `<app-console-shell [section]="section()"
...
- [ ] **Step 2: Run it, verify it fails** — `npx vitest run src/app/console-shell.spec.ts` → FAIL (no component).
- [ ] **Step 3: Minimal implementation** — the component per the Architecture; the section slot
  recipe shares the rail's marker (`after:` 3px bar, `aria-[current=page]:`); `host: { class:
  'contents', '(window:scroll)': 'onScroll()' }`.
- [ ] **Step 4: Run it, verify it passes** — the four files + `src/app/operator/` → PASS.
- [ ] **Step 5: Generalization-audit pass** — population: every consumer of the chip's
  `adminLink` id → `grep -rn "admin-link" frontend/src frontend/e2e`.
- [ ] **Step 6: Commit** — `Add the console shell with its section row and rail (#1011)`.
...
- [ ] **Step 1: Write the failing tests** — `app.spec.ts`: a stub route `operator/:venueId` with
...
- [ ] **Step 2: Run it, verify it fails** — `npx vitest run src/app/app.spec.ts src/app/operator/operator-console.spec.ts` → FAIL.
- [ ] **Step 3: Minimal implementation** — App's walk + mount; the console template.
- [ ] **Step 4: Run it, verify it passes** — the two files + `src/app/operator/` → PASS; then the
...
- [ ] **Step 5: Generalization-audit pass** — population: every e2e that locates the console's
  header, rail, footer or main → `grep -rln "oc-header\|oc-tabs\|oc-footer\|oc-main" frontend/e2e`.
- [ ] **Step 6: Commit** — `Mount the console shell for the venue console (#1011)`.
- [ ] **Step 7: Update plan-doc execution status.**
...
- [ ] **Step 1: Write the failing tests** — `admin-console.spec.ts`: no `nav` in the host when
  authorized either; `console-shell.spec.ts`: the admin rail renders only past the gate.
- [ ] **Step 2: Run it, verify it fails** — `npx vitest run src/app/admin/admin-console.spec.ts src/app/console-shell.spec.ts` → FAIL.
- [ ] **Step 3: Minimal implementation** — the route flag, the console without the strip, the shell's admin branch.
- [ ] **Step 4: Run it, verify it passes** — the two files + `src/app/admin/` → PASS; e2e
  `console-shell admin-console-tabs touch-targets-admin`.
- [ ] **Step 5: Generalization-audit pass** — population: every e2e reading `opc-*` →
  `grep -rn "opc-" frontend/e2e frontend/src` (rewritten in phases 3–5, none left at contract).
- [ ] **Step 6: Commit** — `Mount the console shell for the admin routes (#1011)`.
- [ ] **Step 7: Update plan-doc execution status.**
...
- [ ] **Step 1: Write the failing tests** — `app.spec.ts` route-flag case: `operator` and
...
- [ ] **Step 2: Run it, verify it fails** → FAIL on the two plain routes.
- [ ] **Step 3: Minimal implementation** — the two route entries; `OperatorHome` without the pin.
- [ ] **Step 4: Run it, verify it passes** — `app.spec.ts`, `operator-home.spec.ts`; e2e `console-shell theme-shell operator-password operator-onboarding`.
- [ ] **Step 5: Generalization-audit pass** — population: every self-pinned porcelain host →
  `grep -rn "data-riv-theme" frontend/src/app --include=*.ts --include=*.html | grep -v spec` → only App's binding and `core/theme.ts` remain.
- [ ] **Step 6: Commit** — `Mount the console shell for the operator landing and password page (#1011)`.
- [ ] **Step 7: Update plan-doc execution status.**
...
- [ ] **Step 1: Write the failing test** — `app.spec.ts`: a route carrying only the retired
  `operatorChrome: true` renders the tourist chrome (the flag is dead).
- [ ] **Step 2: Run it, verify it fails** → FAIL (the flag still works).
- [ ] **Step 3: Minimal implementation** — the deletions and the walk without the flags.
- [ ] **Step 4: Run it, verify it passes** — `npm run lint`, `npm run format:check`, `npm test`,
  the mocked e2e files touched; `node scripts/check-plan-file-structure.mjs --diff origin/main`.
- [ ] **Step 5: Generalization-audit pass** — population: every reference to the retired
...
- [ ] **Step 6: Commit** — `Retire the operator chrome and the console's own header (#1011)`.
- [ ] **Step 7: Update plan-doc execution status; push the branch for the maintainer's PR.**
...
- [ ] **AC-1 … AC-8:** `npm test` + the mocked e2e files named per AC → PASS. Verified at commit `<sha>`.
- [ ] **AC-9:** `riviera-docs-freshness` over the PR range → 0 open findings.
