...
| F-13 | Review gate (a11y) | The two new field errors are `role="alert"` siblings with no `aria-describedby` from their field, so a screen-reader user tabbing back later gets no restatement | **deferred — follow-up #821; since discharged.** Repo-wide at the time: the inline field errors all did it this way and none associated. Fixing two of them here would have been a one-off inconsistency, not an improvement. **Closed by #821 (PR #823)**, which swept the whole population rather than these two: one shared `[appFieldErrorFor]` directive (`frontend/src/app/shared/field-error-for.ts`) applied at all **17** field-scoped sites. The "24" counted here was itself an over-count — #821 re-enumerated by mechanism (`role="alert"`, not `*-error` testids) and found 17 field-scoped against 48 form-, page- or action-level, each exclusion recorded in that plan's Non-goals |
