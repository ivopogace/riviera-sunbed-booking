...
| R-11 | `venue.rating_tenths` gains a second writer unnoticed (no machine rule guards the venue table the way `ResponsibilitiesArchitectureTests` guards `set_availability`) | low | med | review-checked boundary: `review` has no SQL touching `venue`; called out for RV-BE; RESPONSIBILITIES §venue gains the "I store the aggregate; `review` computes it" line | impl | **closed — machine-checked** (maintainer finding F-6: `ResponsibilitiesArchitectureTests` gained the rating-columns and review-table sole-writer scans, fixture-proven; RESPONSIBILITIES §machine-checked updated) |
...
| F-6 | Maintainer review (2026-08-30) | The two sole-writer boundaries this slice created (`review` table; `venue.rating_tenths`/`reviews_count`) were left review-checked — "no illegal import, only its own SQL" was an admitted hole (R-11) | **fixed** — two new `ResponsibilitiesArchitectureTests` rules (SQL-shaped review-table scan; rating-columns whole-word scan), each with a vacuous-green guard and rogue/owner fixture proofs; RESPONSIBILITIES.md machine-vs-review-checked updated. A companion maintainer finding ("§review missing from RESPONSIBILITIES.md") did not reproduce at the branch head — §review exists with full Job / Not-My-Job |
...
- `platform/src/test/java/ai/riviera/platform/ResponsibilitiesArchitectureTests.java` — F-6: the review-table and rating-columns sole-writer rules
- `platform/src/test/java/ai/riviera/responsibilityfixture/rogue/adapter/out/{RogueReviewTableReader,RogueRatingColumnWriter}.java` — F-6 red-case fixtures
- `platform/src/test/java/ai/riviera/responsibilityfixture/review/adapter/out/FixtureJdbcReviews.java` — F-6 owner-exclusion fixture (review)
- `platform/src/test/java/ai/riviera/responsibilityfixture/venue/adapter/out/FixtureVenueRatingWriter.java` — F-6 owner-exclusion fixture (venue)
