...
**Stage pointer:** `merge — PR #1052 ready for review; review gate run, Sonar list cleared (F-12, F-13); CI + Sonar re-read on the final head, then merge`
...
| F-13 | Sonar (PR analysis on 7d1e0f60): 0 issues, but `new_duplicated_blocks` = 2 | the `LayoutRejection` → problem map lived twice, in `VenueAdminController` and `RemodelCommitController` | fixed — the rejection carries its `Fault` and `detail`; both routes read them off the enum |
...
- [x] **Close-out written in THIS PR, in its last code-touching commit** (the F-13 duplication fix; merged via PR #1052) — the plan doc's final state is committed here, citing `merged via PR #NN`, and no docs-only commit follows it.
