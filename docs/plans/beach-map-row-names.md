...
- [ ] **AC-5:** Given a venue loaded with descriptive labels (e.g. the V3 seed shapes),
...
- [ ] **AC-6:** Given two rows given the same trimmed name, when the operator tries to
...
- [ ] **AC-7:** Given named rows, when the operator regenerates the grid (confirmed
...
- [ ] **AC-8:** Given a venue whose row already carries a custom name, when a set is
...
- [ ] **AC-9:** Given confirmation-mail facts with `rowLabel` "Front row · Sea view",
...
|---|---|---|---|---|---|---|
| R-1 | Flyway `V43` collision with in-flight work | low | high | Grill verified: highest on `main` is `V42`; only dependabot PRs open. If a collision appears, this branch renumbers (merges second). | session | open |
| R-2 | `CHECK` fails on existing rows | low | high | Longest existing label is the V3 seed's 20-char "Front row · Sea view" < 40; `BeachMapLayoutMigrationIT` migrates the real chain against the seed. | session | open |
| R-3 | Java length check disagrees with PG `char_length` (UTF-16 vs code points) | med | med | Validate with `codePointCount` (== `char_length` semantics); FE `maxlength` counts UTF-16 units (equal or stricter — never lets through what the server rejects for BMP text; astral-plane over-strictness accepted). | session | open |
| R-4 | Duplicate row names → confusing server `DUPLICATE_POSITION` ("two sets overlap") | med | low | Client-side duplicate guard with row-name copy (AC-6); server refusal stays the backstop. | session | open |
| R-5 | Mail template change breaks `SmtpMailerIT` / resend path silently | low | med | The IT pins the exact line; update the assertion in the same TDD step; resend shares `SmtpMailer`. | session | open |
| R-6 | Pricing tab keys rows by label (URL path `rows/{rowLabel}/price`) — renames change row identity | low | med | Renames only land through the bulk replace; the console re-reads the map after save; the reprice URL is already encoded (`operator-console.service.ts:156`) and the seed's `·`-labels already exercise it. No code change; verified in e2e. | session | open |
| R-7 | BOLA (invariant #13) | — | high | No new endpoint; `replaceLayout` already runs `VenueOwnership.assertOwns` in the application service (pinned by `CrossVenueDenialIT`). Unchanged. | session | open |
| R-8 | Error contract drift (§6b) | low | med | The bound throws `IllegalArgumentException` inside `SetCommand`, already wrapped by `InvalidApiRequestException.parsing` → `400 INVALID_REQUEST`; no controller or handler change. | session | open |
...
- **Assumption:** dropping the mail's literal `Row ` prefix (AC-9) is acceptable copy —
  bare-letter venues read `Spot: A, position 3`. — *Owner:* session · *Resolves by:*
  phase 2; flagged for the maintainer in the PR description.
- **Assumption:** mid-season renames (sold venues) are out of scope; a follow-up issue
  is filed at merge close-out. — *Owner:* session · *Resolves by:* close-out step.
...
- [ ] **Step 1: Red** — `SetCommandTest`: a 41-code-point label throws; a 40-code-point
...
- [ ] **Step 2: Green** — `VenueFieldValidation.requireText(String, String, int)` using
  `codePointCount`; `MAX_ROW_LABEL_LENGTH = 40`; `SetCommand` calls it. → PASS.
- [ ] **Step 3: Red** — IT cases: `overlongRowLabelIs400` (modelled on `blankNameIs400`),
  ...
  without). → PASS.
- [ ] **Step 4: Regression** — `./gradlew test --tests "*venue*"` (module scope) + the
  structural net if any file moved (none expected).
- [ ] **Step 5: Generalization audit** — population: "every write path that persists
...
- [ ] **Step 6: Commit** — `Bound beach-map row labels at 40 characters (#723)`.
...
- [ ] **Step 1: Red** — `SmtpMailerIT`: confirmation with `rowLabel` "Front row · Sea
  view" must render `Spot: Front row · Sea view, position 3` (no `Row Front row`).
  Run: `./gradlew test --tests "*SmtpMailerIT*"` → FAIL.
- [ ] **Step 2: Green** — drop the literal `Row ` from the text block. → PASS.
- [ ] **Step 3: Generalization audit** — population: "every mail body rendering
...
- [ ] **Step 4: Commit** — `Render the spot line from the row label alone (#723)` +
  Execution status.
...
- [ ] **Step 1: Red** — unit specs for AC-4 (named row saves), AC-5 (loaded labels
...
- [ ] **Step 2: Green** — `rowNames` signal seeded in `seedFrom` (first set per `gridY`),
...
- [ ] **Step 3: Lint/format** — `npm run lint && npm run format:check`.
- [ ] **Step 4: Commit** — `Let operators name beach-map rows in the layout editor (#723)` +
  Execution status.
...
- [ ] **Step 1: Red** — `set-editor.spec.ts`: adding a set at a `gridY` whose loaded
  sets carry "Under the pines" uses that label; an empty row falls back to the grid
  letter. → FAIL.
- [ ] **Step 2: Green** — thread the loaded sets into the placement derivation. → PASS.
- [ ] **Step 3: e2e** — extend `layout-editor.e2e.ts`: name a row, save, assert the PUT
  body labels; duplicate names surface the guard message. Run
  `npm run test:e2e:a11y -- layout-editor` (mocked suite). → PASS.
- [ ] **Step 4: Generalization audit** — population: "every FE site that derives a
...
- [ ] **Step 5: Commit** — `Per-set adds inherit the row's name (#723)` + Execution status.

## Phase 5 — Gates + close-out

- [ ] CI green on the PR head; mark ready for review (merge latest `origin/main` first,
  with the routing gate for whatever the integration touches).
- [ ] Review gate (`/code-review` ladder + `riviera-review-overlay`); findings re-enter
  at Implement.
- [ ] Sonar gate: pull the new-issue + duplication list from the API; clear it.
- [ ] `node scripts/check-plan-file-structure.mjs --diff origin/main` (plan doc staged).
- [ ] Merge close-out per `references/pr-gates.md` §3, incl. `riviera-docs-freshness`
...
- [ ] **AC-1..3:** `./gradlew test --tests "*SetCommandTest*" --tests "*VenueAdminControllerIT*" --tests "*BeachMapLayoutMigrationIT*"` → PASS. Verified at commit `<sha>`.
- [ ] **AC-4..7:** `npm test -- layout-editor` → PASS. Verified at commit `<sha>`.
- [ ] **AC-8:** `npm test -- set-editor` → PASS. Verified at commit `<sha>`.
- [ ] **AC-9:** `./gradlew test --tests "*SmtpMailerIT*"` → PASS. Verified at commit `<sha>`.
...
- [ ] **Availability** section filled; no availability write path touched (invariant #2).
- [ ] Pool + cutoff rules honored (invariants #3, #4) — untouched.
- [ ] **Modulith** section filled; no cross-module `application.*`/`adapter.*` imports (invariant #11).
- [ ] **Payment/payout** N/A holds — no money semantics changed.
- [ ] Refund policy untouched (invariant #10).
- [ ] Timezone untouched (invariant #6).
- [ ] Booking codes untouched (invariant #7).
- [ ] Flyway migration present + CHECK tested (invariant #12).
