...
merged) — § Round 7 § 3 (the crowd-mean rule), § Round 10 § 3 (the
...
`riviera-plan-doc` (forced the ACs to name seams, which moved the foot swap out of `home.ts`
into a pure function) · `tdd` (each phase red-first at
...
CI-safe mocked suite) · `domain-modeling` (owns `CONTEXT.md`: the desktop surface is its own glossary term, **venue
panel**, not four clauses of behaviour bolted onto the sheet head's entry) · `riviera-tailwind`
(dusk is three utilities on the fixed solid-button pair,
...
- [x] **AC-1:** Given a crowd whose first member is narrower than its widest and offset from the
      crowd's mean, when a further pin is tested for membership, then it is tested against the crowd's
      running mean and widest member, so the shore's chain groups into one crowd. _Seam:_
      `pin-crowding.ts` `crowdPins` · _Pinned by:_ `pin-crowding.spec.ts` › `crowdPins` ›
      `tests a pin against the crowd’s running mean, not its first member`,
      `tests a pin against the crowd’s widest member, not its first`
- [x] **AC-2:** Given a pill that fits at none of centred, hung right or hung left, when it is laid
      out, then it is tried below, above and at the four diagonals (a 32 px hang, a 10 px gap from the
      point) and then over the nine spots again as a bare disc, before it collapses. _Seam:_
      `pin-crowding.ts` `layoutPills` · _Pinned by:_ `pin-crowding.spec.ts` › `layoutPills` ›
      `hangs a pill below its point when every level spot is taken`,
      `hangs a pill diagonally before collapsing it`,
      `tries the nine spots again as a bare disc before it stands its ground`
- [x] **AC-3:** Given no-go boxes for Near me and the tourist's dot and a window narrower than the
      ...
      being recentred elsewhere. _Seam:_ `pin-crowding.ts` `layoutPills(crowds, fullWidth, space)` ·
      _Pinned by:_ `pin-crowding.spec.ts` › `layoutPills` ›
      `hangs a pill clear of the page’s chrome at the map’s foot`,
      `hangs a pill clear of the tourist’s own dot`,
      `confines a pill to the window, which need not start at the box’s own top`,
      `leaves a pill that fits nowhere at its layer placement`
- [x] **AC-4:** Given a crowd of which one member has closed and a lone `Borsh` whose sales have,
      when the layer draws them, then `Borsh` carries `data-dusk` and the pill does not, and the dusk
      skin is desaturation on the fixed hover fill with the price struck and the ink untouched.
      _Seam:_ `app-venue-pin-layer` rendered output · _Pinned by:_ `venue-pin-layer.spec.ts` › `dusk`
      › `greys a crowd only when every member has closed`,
      `greys a lone pin whose own sales for the day have closed`,
      `strikes the price and leaves the name, so dusk is never carried by colour alone`;
      `venue-pin-layer.contrast.spec.ts` ›
      `the dusk face clears AA once the saturate filter has been applied to both`,
      `the dusk count disc, which inverts the same pair, clears AA too`
- [x] **AC-5:** Given a lone pin under the foot row's right-hand spot and none under the mirrored
      left-hand spot, when the foot's placement is decided, then Near me and the credit swap sides
      together; given a lone pin under both, then nothing moves (it cannot oscillate). _Seam:_
      `pin-crowding.ts` `footSwap` · _Pinned by:_ `pin-crowding.spec.ts` › `footSwap` (five cases:
      `swaps the foot when a lone pin sits under it and the mirrored spots are free`,
      `holds when a lone pin waits under the swapped spots too, so it cannot oscillate`,
      `holds when nothing is under the foot at all`,
      `swaps back once the pin that moved the chrome has gone`,
      `holds when the foot has not been drawn yet`)
- [x] **AC-6:** Given the mocked fixture at 390, 430, 768, 1024, 1440 and 1920, located and not,
      when the page has settled, then the count of intersections between a rendered place pill or lone
      pin and any rendered map chrome box is **0** at every width. _Seam:_ the rendered page ·
      _Pinned by:_ `frontend/e2e/discover-map.e2e.ts` › `Discover map — the flag’s pins keep off the chrome`
      › `no pin face sits on the map chrome at <w> × <h>, placed or not` (six widths)
- [x] **AC-7:** Given `?map=sheet` at 1440 × 900, when the page renders, then the panel is 540 px
      wide with its first row at y 191, the map is inset 12 px on all four sides with 22 px corners and
      the pane is 864 px; at 1024 the panel is 420 and the pane 568, with the shore's chain grouped
      into one `Palasë & Dhërmi` pill; at 1920 the pane is 1,344. _Seam:_ the rendered page ·
      _Pinned by:_ `frontend/e2e/discover-map.e2e.ts` › `Discover map — the desktop panel` ›
      `at <width> the panel is <panelPx> and the map takes the remaining <panePx>` (three widths),
      `the panel opens on its head with the first row where the phone’s sheet puts it`,
      `the map’s corners match the panel’s at 22 px, on all four`,
      `the shore’s chain groups into one pill, which the first-member rule would have split`
- [x] **AC-8:** Given a venue row selected on the desktop panel, when it expands to its amenity
      chips and its non-default booking mode, then it measures 121 px against every other row's 92, and
      the number of rows visible is unchanged. _Seam:_ the rendered page · _Pinned by:_
      `frontend/e2e/discover-map.e2e.ts` › `the selected row expands, and no other row moves or leaves the panel`;
      the content rule in `venue-row.spec.ts`
- [x] **AC-9:** Given a region of more than 15 venues, when the panel's list is scrolled past a
      beach head, then the head sticks at the list's top and carries its count; at 15 or fewer the
      desktop head carries no count at all. _Seam:_ the `desk-group` head in `home.html` ·
      _Pinned by:_ `home.spec.ts` › `from lg: the panel` ›
      `sticks the heads past fifteen venues, and only then gives them their counts`,
      `runs the beaches as heads with a rule and no count, while the region fits the panel`
- [x] **AC-10:** Given the desktop panel at 1440, when the place button is pressed, then the coast
      picker opens as a popover anchored 8 px under it, 420 px wide, with its ribbon 150 px tall.
      _Seam:_ the rendered page · _Pinned by:_ `frontend/e2e/discover-map.e2e.ts` ›
      `the coast picker opens as a popover under the place button, with its ribbon`; `home.spec.ts` ›
      `hangs the coast picker off the place button, which is the desktop’s only chooser`
- [x] **AC-11:** Given the flag off, when Discover renders at any width, then the filter bar, the
      view switch and the shipped map panel are exactly what they are on `main`. _Seam:_ the rendered
      page · _Pinned by:_ `frontend/e2e/discover-map.e2e.ts` ›
      `the flag off leaves today’s Discover at the same width` and the existing
      `shows the list and the map side by side on a wide screen, with no switch`
- [x] **AC-12:** Given the desktop panel, when axe runs and every control's box is measured, then
      there are no serious violations and nothing is under 44 × 44 px. _Seam:_ the rendered page ·
      _Pinned by:_ `frontend/e2e/discover-map.e2e.ts` ›
      `is axe clean with every control at the touch floor, listing and with the picker open`;
      `home.a11y.spec.ts`, `home.contrast.spec.ts`, `venue-pin-layer.a11y.spec.ts`
...
> _changes_ is the pill layout, which is behind the same flag only in its inputs — the nine spots
...
| The sheet's beach heads scroll away with their rows          | Changed   | They stick at the sheet's top past 15 venues, as the panel's do: README § Round 11 § 5 asks it of both surfaces. The sheet's head keeps the count it always carried; only the desktop head earns one at 15. |

## Risk register

| #   | Description                                                                                                                                                                                 | Likelihood | Impact            | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Owner | Resolution |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------- |
| R-1 | Measuring the chrome in an `afterRenderEffect` and publishing it as a signal the layer consumes could loop: the layer re-lays-out, which re-renders, which re-measures                      | Medium     | High (a hung tab) | The measurement tracks only a `chromeTick` counter, bumped by a plain `effect` reading the things that genuinely move the chrome (detent, foot bottom, located state, viewport, swap). Pills moving bumps nothing. Pills moving bumps nothing, so the measurement cannot re-enter itself; `home.spec.ts` › `hands the layer the panel’s own Near me, which the map component does not draw` renders the panel to settled boxes, which a loop would never reach | plan  |            |
| R-2 | The foot swap oscillates: swapping puts a lone pin under the mirrored spot, which swaps back                                                                                                | Medium     | High              | `footSwap` is pure and decides from the CURRENT rendered boxes and their mirror: it swaps only when the current spots are covered AND the mirrored ones are not; when both are covered nothing moves. Pinned by AC-5's `holds when a lone pin waits under the swapped spots too, so it cannot oscillate`                                                                                                                                                       | plan  |            |
| R-3 | Round 12's "6 rows visible at 1440, 7 at 1920" cannot be reproduced without the prototype's own fixture, so an e2e asserting the literal counts would pin the fixture, not the design       | High       | Medium            | The e2e asserts the load-bearing claim instead — the selected row is the only one off 92, at 121 (29, not the record's 26: the shipped amenity chip is 26 px tall where the prototype's was 23), and the count of whole rows inside the scroller is the same before and after (AC-8)                                                                                                                                                                           | plan  |            |
...
- **Assumption:** `VenueCard.salesClosed` is the right dusk signal for "sales for today have
  closed" — it is the server's verdict for the _chosen_ day, and the design record's dusk case is
  `?now=16:30` on today. A tourist who picks tomorrow sees tomorrow's verdict on both the pin and
  the row, which is the consistency the shared card record exists for. — _Owner:_ plan ·
  _Resolves by:_ AC-4 and the list card's existing `Closed today` chip agreeing in `home.spec.ts`
...
- **Open question (D-2):** The issue's desktop pane widths (580 at 1024, 1,356 at 1920) and its
  ...
  numbers are stale; noted on #1159. _Outcome:_ AC-7.
- **Open questions (D-3, D-4):** The two narrow-pane items the issue carries. — **Dropped**,
  confirmed by the maintainer; the arithmetic is on #1159.
- **Open question (D-5):** Is the previous slice's plan doc (`docs/plans/map-poster.md`, #1158,
  merged via PR #1163) due for retirement? — **Yes**, `riviera-docs-freshness` § _Plan-doc
  retirement_ deletes a merged plan at the next close-out, and this is it. _Outcome:_ phase 7.
...
(`VenueCard.salesClosed`, `VenueCard.freePercent`). Invariant **#4** is _rendered_, not changed:
...
| FE-3 | `pages/home/venue-row.ts` / `.html`       | **New**      | Component   | `input()` card, selected, date; `output()` pointed                                                                                         | —     |
...
**Next action:** none — PR #1164 closes #1159. Close-out is this commit: docs-freshness over
`bb0e0785..HEAD`, and `docs/plans/map-poster.md` (#1158, merged via PR #1163) retired per D-5.
...
| 6 — The mocked e2e: intersections, the panel's geometry, the popover                  | ✅     | `2a9daf47`  |
| 7 — a11y, contrast, touch targets, close-out                                          | ✅     | this commit |
...
- `frontend/src/app/pages/home/discover-head.ts` — the place button's popover anchor and its
  projected slot
...
- [x] `data-dusk` on a pill whose every member has closed and on a lone pin that has; the skin is
      `saturate-0` + the fixed hover fill + the price struck, ink untouched.
- [x] Commit — `Grey a place pill only when every venue in its crowd has closed (#1159)`
...
- [x] `RivieraMap.chromeBoxes()`; `home`'s `chromeTick` + `earlyRead` measurement; the sheet's
      window (header → the sheet's rest); `footSwap` driving which side Near me and the credit take.
- [x] Commit — `Keep the phone's pills off the map chrome, and move the chrome for a lone pin (#1159)`
...
- [x] Commit — `Put the sheet's list in a pinned left panel beside an inset map from lg (#1159)`
...
- [x] Commit — `Light a row from its pin and a pin from its row on the desktop panel (#1159)`

## Phase 6 — The mocked e2e

**Files:** Modify `frontend/e2e/discover-map.e2e.ts`

- [x] Commit — `Count every pill-on-chrome intersection across the fixture regions (#1159)`

## Phase 7 — Close-out

**Files:** Modify `docs/plans/pin-layer-placement.md` · Delete `docs/plans/map-poster.md`

- [x] `riviera-docs-freshness` over `origin/main..HEAD`; the epic checklist ticked; #1159 closed.
...
Run at the close-out commit, from `frontend/`, with `PW_CHROMIUM_EXECUTABLE=/opt/pw-browsers/chromium`
(the image's Chromium; the session must never run `playwright install`).

| Command                                                         | Covers                                                     | Result           |
| --------------------------------------------------------------- | ---------------------------------------------------------- | ---------------- |
| `npm test`                                                      | AC-1…AC-5, AC-9, AC-12 (unit)                              | 3617 / 293 files |
| `npm run test:e2e:a11y`                                         | AC-6…AC-12 (browser)                                       | see below        |
| `npm run lint`                                                  | the repo's ESLint rules                                    | clean            |
| `npm run format:check`                                          | Prettier over `src e2e eslint-rules vitest-base.config.ts` | clean            |
| `node scripts/check-inline-comments.mjs --diff origin/main`     | RV-STYLE-1's mechanical half                               | clean            |
| `node scripts/check-touch-target.mjs --diff origin/main`        | the 44 px declaration                                      | clean            |
...
- [x] No JPA (#1). Availability N/A justified; nothing touches `set_availability`.
- [x] Pool + cutoff honoured (#3, #4 — rendered, not changed). Money minor units (#5). UTC stored,
      `Europe/Tirane` reasoned (#6). Codes unguessable (#7).
- [x] Modulith section N/A justified; no backend file in the diff.
- [x] Payment section N/A justified.
- [x] No Flyway migration (#12) — none needed.
- [x] Frontend standards met: every new control declares the 44 px floor, no `outline-none`, tokens
...
- [x] The review gate ran in full (ladder in `riviera-sdlc` `references/pr-gates.md` §1 plus the
