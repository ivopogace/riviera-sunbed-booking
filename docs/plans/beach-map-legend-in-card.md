...
- [x] **AC-3:** Given a venue whose row 5 is the walk-in pool, when the map renders, then
      ...
      `venue-map.contrast.spec.ts › 'the walk-in numeral meets AA on both hatch bands over every wash stop (#701)'`
- [x] **AC-4:** Given the rendered map, when the legend swatches and the tiles are compared
      by computed style, then each swatch's `background-image`/`background-color`/`border-color`/
      `border-style` equal its tile's — and, because a translucent fill only _looks_ alike over the
      same ground, the band itself is painted the wash's own first stop. _Pinned by:_
      `venue-map-pan.e2e.ts › 'every legend swatch declares exactly what the tile it stands for declares (#701)'` + `venue-map-pan.e2e.ts › "the legend band is painted the wash's own first stop, so swatches share the tiles' ground (#701)"` + `map-tile.spec.ts › 'renders each state from the one shared appearance record, and nothing besides'` + `venue-map.spec.ts › 'lets the appearance directive own every tile colour — the template adds none (#701)'`
- [x] **AC-5:** Given a `BeachMapCanvas` host that projects **no** `canvasLegend` content
      (every operator surface), when it renders, then the legend band generates no box at all and
      the wash still meets the sea banner — the operator layout editor, Daily view and per-set
      editor render as they did on `main`. _Pinned by:_
