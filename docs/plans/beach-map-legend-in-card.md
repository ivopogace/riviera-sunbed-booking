...
**Next action:** Phase 2 — write the failing `venue-map.spec.ts` legend-placement spec, then
recompose `venue-map.html` (legend into `canvasLegend`, trailing card out, tiles on
`[appMapTile]`).
