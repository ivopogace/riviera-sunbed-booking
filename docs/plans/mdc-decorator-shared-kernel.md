...
| F-3 | review (`/code-review` fan-out, code-comment-compliance reviewer) | **`RESPONSIBILITIES.md` contradicted itself inside the `shared` section.** The generalizing paragraph this PR added ("no admission here has ever rested on reuse … the two newest") sat four lines below the surviving sentence "this is **the one** admitted type whose justification is *not* 'more than one module needs it'". Both cannot be true. D-5 above recorded this as patched when only the addition had landed | **fixed** — the metric-names clause now states its own distinct ground (admitted for *naming-convention consistency*, since all six counters have a single reader) rather than claiming to be the only non-reuse admission; re-swept, the block is now internally consistent |
| F-4 | review (`/code-review` fan-out, code-comment-compliance reviewer) | **A dangling `{@link MdcTaskDecorator}` in `AsyncMailDispatcherTest:399`.** That file sat in the decorator's *old* package, so it referenced the type with no import; every other referencing site needed an import updated and got one, but this reference is **Javadoc-only**, so nothing failed to compile and the move left it unresolvable | **fixed** — added `import ai.riviera.platform.shared.MdcTaskDecorator;` (the file already imports `shared.ObservabilityMetrics`, and `notification → shared` is the permitted direction). Tests re-run green |
...
| D-5 | `RESPONSIBILITIES.md` non-reuse note | "this is **the one** admitted type whose justification is *not* 'more than one module needs it'" | `ShutdownBudget` and now `MdcTaskDecorator` are both admitted on ownership | **patched, but only half of it at first** — the generalizing paragraph was added while the "the one" sentence four lines above was left standing, so the section contradicted itself. The review gate caught the remainder as **F-3**; the metric-names clause now states its *own* distinct justification (naming-convention consistency) instead of claiming uniqueness |
