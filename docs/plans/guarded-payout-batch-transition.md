...
6 findings, all 6 fixed — F-4 and F-6 were first deferred/skipped, then called in by the maintainer; see the findings register) · `riviera-docs-freshness`
...
| F-4 | review (high) | No test exercises the lost-race path end to end: the guard is proven at the adapter, the mapping only against mocks | fixed in `4692242b` (initially deferred to #653; the maintainer called it in) — `PayoutBatchRaceIT` stubs the pre-read stale via `@MockitoSpyBean` and lets the guarded write and the re-read hit real Postgres. Mutation-checked: with `AND status = :expected` removed it goes red, so it detects the defect it exists for |
