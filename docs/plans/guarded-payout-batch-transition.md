...
6 findings, all 6 fixed — F-4 and F-6 were first deferred/skipped, then called in by the maintainer. A medium-effort re-review of that fix round found 4 more (F-8), all fixed; see the findings register) · `riviera-docs-freshness`
...
| R-4 | Sonar S1192 (duplicated string literals) on the new named params | med | low | Bind through `private static final` param-name constants, matching `JdbcBookings`/`JdbcPayments` (`riviera-java-conventions` §6a) | Ivo | closed — `PARAM_ID` + `PARAM_PERIOD` hoisted at the maintainer's call (F-6), matching `JdbcPayments`/`JdbcBookings`, though ×2 each sits under S1192's threshold of 3 |
...
| F-4 | review (high) | No test exercises the lost-race path end to end: the guard is proven at the adapter, the mapping only against mocks | fixed in `4692242b` (initially deferred to #653; the maintainer called it in) — `PayoutBatchRaceIT` stubs the pre-read stale via `@MockitoSpyBean` and lets the guarded write and the re-read hit real Postgres. Mutation-checked: with `AND status = :expected` removed it goes red, so it detects the defect it exists for. **Correction (F-8):** `4692242b`'s commit message claims this also pins F-2's READ COMMITTED dependency. It does not — the spy makes the pre-read stale synthetically, so the row is already committed at its final status before the transaction opens and every isolation level would show it. The claim is retracted in the test's own Javadoc; pinning isolation needs a second connection committing mid-transaction, which nothing here does |
...
| F-6 | review (high) | SQL bind names inline where sibling adapters hoist `PARAM_*` constants | fixed in `4692242b` (initially skipped as under S1192's threshold; the maintainer called it in) — `PARAM_ID` and `PARAM_PERIOD` hoisted, matching `JdbcPayments`/`JdbcBookings`. The two repeated names are the ones hoisted; the single-use ones stay inline, as in the sibling adapters (also R-4) |
| F-8 | review (medium, re-review of the F-4/F-6 round) | `PayoutBatchRaceIT`'s Javadoc and one assertion message claimed it pins `mark`'s READ COMMITTED dependency; it doesn't, because the stale read is injected by a spy rather than produced by a concurrent commit | fixed in `db0522b5` — both claims retracted at source, and the Javadoc now states what the test does *not* cover. Three siblings from the same re-review fixed with it: a no-op `doCallRealMethod()` stub, a `staleDraft` fixture that hardcoded the wrong venue id, and `settledBatch()`→`newBatch(status)` so each test names its own precondition instead of inserting-then-overwriting |
...
- `platform/src/test/java/ai/riviera/platform/payout/PayoutBatchRaceIT.java` — NEW: the lost-race path end to end — stale pre-read, real guarded write, real re-read (review finding F-4)
