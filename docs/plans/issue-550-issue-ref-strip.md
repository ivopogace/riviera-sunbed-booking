...
| B-0 | 5-file backend-test mini-probe (R-4 rate) | — | ✖ won't do (pass stopped after Phase A; fires only on revival) |
| B-1… | backend test, per the mini-probe's verdict | — | ✖ won't do (same) |
| C-0 | 5-file backend-main mini-probe (R-4 rate) | — | ✖ won't do (same) |
| C-1… | backend main | — | ✖ won't do (same) |

Legend: blank = not started, ⏳ = in progress, ✅ = done, ✖ = won't do (maintainer stop decision).
...
- `platform/src/test/` — planned as Phase B — **never executed** (B/C stopped by maintainer decision)
- `platform/src/main/` — planned as Phase C — **never executed** (B/C stopped by maintainer decision)
