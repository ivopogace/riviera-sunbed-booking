...
`riviera-sdd` routing gate) so every future cross-module driven port follows it automatically.
...
**Branch:** `claude/riviera-sdd-issue-44-molx4o` (harness-designated; continuing on it — this is a
...
- [ ] **AC-5 (workflow):** The api-vs-spi rule is recorded in `CLAUDE.md` #11, the
  ...
  `riviera-sdd` routing gate. *Pinned by:* the diff + RV-PROC-1 self-check.
...
| 1 — codify the rule (CLAUDE.md, riviera-modulith, review overlay, riviera-sdd) | ✅ | (this slice) |
| 2 — verify (ModularityTests + ITs + full suite) + review gate | ✅ | (this slice) |

Legend: blank = not started, ⏳ = in progress, ✅ = done.

> **Review-gate outcome (SDD):** `riviera-review-overlay` + a focused independent reviewer over the
...
- [ ] Rule recorded in CLAUDE.md #11, `riviera-modulith`, `riviera-review-overlay` bank, and the
  `riviera-sdd` routing gate (RV-PROC-1 stays truthful).
