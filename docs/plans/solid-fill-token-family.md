...
| close-out | **The counting sweep** (`riviera-docs-freshness` 2b) — this slice makes the **third** theme-invariant family, so any "the two X" about them would now be false | a repo-wide grep for `the/both/only/first/second two` across the substrate docs and `frontend/src`, narrowed to `theme|invariant|token|famil|fill|ink|solid` | **Zero findings.** 27 hits, all read: every "two" is of a different subject (the chip families, the map ink families, the outline borders, the two reds) and stays true. Two near-misses ruled out — `riviera-frontend`'s "the token registry lives in two places, and only two" survives because `core/theme.ts:8` is a **theme** registry, not a per-token one and this slice adds no theme; and the ledger's class-F "must move as a pair" describes class F, while this family is class R. |
