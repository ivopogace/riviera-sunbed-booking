...
| 4 — Import cleanup (F-3, maintainer-requested) | ✅ | `369218b` |
...
| F-4 | CI (GitHub Actions incident) | After the F-3 push, GitHub Actions stopped creating check suites **repo-wide** for ~9 hours (last dispatch 17:54Z, recovery ~03:05Z) — so the final head could not be verified, and the PR sat `mergeable_state=blocked` with zero check-runs. Ruled out as ours at the time: the diff touches no `.github/` file, all four workflows read `state=active`, and the Sonar/Render/Claude apps kept registering suites on every push, so webhooks were healthy and only Actions was refusing work. Confirmed as a platform incident on githubstatus.com. | **closed — external, no code implicated.** Cancelling superseded runs, a PR close/reopen and an empty commit (`3a53f43`) all failed to dispatch while the incident ran; the *same* close/reopen dispatched within seconds once it cleared, and every check then passed first time. The empty commit is retained rather than rewritten — force-pushing a branch mid-incident had more downside than a tidy log was worth. |
| F-3 | review gate (out-of-scope observation, then maintainer instruction) | `WebSliceStubs.java` carried the same `import ai.riviera.platform.shared.ResubmissionOutcome;` **11 times** — legal Java, so it compiled and no linter caught it; almost certainly a bad sweep during the #371 `shared`-kernel extraction or #454. | **fixed in `369218b`** — 10 duplicates removed and the survivor moved into the file's existing alphabetical `shared` block (after `CurrentOperator`), where the other two `shared` imports already sat; the strays had been wedged between the `notification` and `operator` groups. Import count 131 → 121. Verified by the 10 `WebSliceStubs`-dependent web-slice tests. |
...
| Gate | Outcome |
|---|---|
| CI | Green on the final head `3a53f43` — backend, frontend, repo hygiene (both diff-scoped guards) — after the ~9-hour Actions outage in F-4. **R-3 closed here and only here:** the ~66 probes survived the cached-context full-suite run with no `429`, which a scoped run cannot demonstrate. |
| CodeQL | Green (F-2 — early attempts were queue starvation, never executed; green on the final head `3a53f43`, both languages). |
| Review | Ran at high effort, 6 dimensions. 1 finding (F-1), fixed in `157226e`. |
| Sonar | Green **and its reported list verified empty via the API**, not inferred from the badge: `measures` populated (`new_lines` 15) and the `SonarCloud Code Analysis` check `success`, which together rule out the false-clean read where an unanalyzed PR returns the same `total: 0` as a genuinely clean one. `new_code_smells` 0, `new_duplicated_blocks` 0. Re-verified on the final head after the review-fix and import-cleanup pushes, not just on the first analysis. |
