...
- [x] **AC-7:** Given the whole context under the `mailer` profile, when it starts, then all three
      ...
      + `MailTransportPropertiesTest.theRelayTimeoutsAreTheSameKnobTheDrainIsDerivedFrom` (existing)
...
| — | review gate (reviewers 3/5 history, 5/5 comments) | No findings. 3/5 independently re-derived the value equivalence (`20000/2` → `MAIL_POOL_CLAIM_MS`, `ofSeconds(5)` → `ofMillis(5000)`) and confirmed the untouched `RefundExecutorPropertiesTest` `DEFAULT == MAX` coupling still holds; 5/5 confirmed no Javadoc claim was falsified and zero inline comments were added | passed |
| F-1 | review gate (reviewer 2/5, shallow bug scan) | **The detector keyed pools by SIMPLE name through a `Set<String>`, so it could silently under-count** — the exact failure mode this slice exists to end, reintroduced through the guard's own key. Two collapsing cases: same-named classes in different packages (every module has its own `adapter.in`), and — worse and likelier — **one class configuring a second draining pool**, which adds a real sequential window while the discovered set, the claim linkage and the sum all stay green | **fixed-in-`3ad0c92`** — keys are now fully qualified (closes case 1), **and** a new `everyDrainingPoolDeclaresExactlyOneWindow` counts `setAwaitTerminationMillis` **call sites** rather than classes (closes case 2, which FQN keying alone does *not* — the reviewer's proposed fix would have left it open). Proven non-vacuous by `countingWindowsSeesASecondPoolThatCountingClassesCannot`: the fixture now declares two pools in one class, and the assertions show 1 class / 2 windows |
| F-2 | review gate (reviewer 4/5, prior-PR comments) | **The PR body cited AC-6 as pinned by `RefundExecutorPropertiesTest.rejectsADrainOutsideTheClaim`, which exists nowhere.** The real test is `rejectsADrainThatWouldOutlastTheShutdownGrace`. The plan doc had already been corrected; the PR body drifted from it. This is the defect class PR #433's review named by number (F-2/F-6: an AC pin-name that reads as tested when it isn't) | **fixed-in-`3ad0c92`** — PR body corrected to the shipped name |
| F-3 | review gate (reviewer 4/5, prior-PR comments) | **The plan doc's AC-7 cited `MailTransportPropertiesTest.resolvesEverySmtpTimeoutThroughTheEnvironment`, which exists nowhere** — invented at plan time and never re-verified against the shipped tests. Real coverage is `theRelayTimeoutsAreTheSameKnobTheDrainIsDerivedFrom` | **fixed-in-`3ad0c92`** — AC-7 repointed at the real test. Both F-2 and F-3 are the same root cause: pin-names written at plan time, not re-checked at close-out |
