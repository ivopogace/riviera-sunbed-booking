...
`riviera-docs-freshness` (**due at phase 3** over `origin/main...HEAD`; one finding is already known at
plan time and scheduled there — `CLAUDE.md`'s `notification` row states the registry pool's bounds are
"validated at boot on both ends", which stays true, but `RefundExecutorProperties`' "do that where the
budget is stated, not here" now points at a real place and should name it) · `riviera-modulith`
...
- [ ] **AC-1:** Given the shipped production code, when the guard scans for pools that drain on
...
- [ ] **AC-2:** Given a deliberately oversized draining pool in the fixture tree, when the same
...
- [ ] **AC-3:** Given the claims declared in `ShutdownBudget`, when they are summed, then the total is
...
- [ ] **AC-4:** Given a pool discovered by AC-1, when the guard runs, then that pool has a **declared
...
- [ ] **AC-5:** Given `MailTransportProperties`, when it validates `socketTimeoutMs`, then its ceiling
...
- [ ] **Step 1: Write the failing test** — the non-vacuity case first, because it is also the
...
- [ ] **Step 3: Minimal implementation** — the detector plus the production-side discovery rule.
...
- [ ] **Step 4: Run it, verify it passes** — `gradle --no-daemon --console=plain test --tests
...
- [ ] **Step 5: Generalization-audit pass** — search for other "increment this constant when a new X
...
- [ ] **Step 6: Commit** — `git commit -m "test(#456): discover every pool that drains on shutdown"`
...
- [ ] **Step 1: Write the failing test** — the budget rule's own red case, so AC-3 is falsifiable
...
- [ ] **Step 3: Minimal implementation** — constants + one pure function. Claim values are the
...
- [ ] **Step 5: Wire the guard's remaining two rules** (AC-3, AC-4) — the claim map keyed by the same
...
- [ ] **Step 6: Add R-4's property rule** — the one discovery hole a bytecode scan structurally has,
...
- [ ] **Step 7: Run the class** → PASS; **commit** —
...
- [ ] **Step 1:** Repoint `MailTransportProperties` — delete `MAIL_SHUTDOWN_BUDGET_MS` and
...
- [ ] **Step 2:** Repoint `RefundExecutorProperties.MAX_SHUTDOWN_DRAIN` at
...
- [ ] **Step 3:** Delete `theCombinedDrainOfEveryPoolFitsTheMailShutdownBudget` (the ledger's row 1 —
      it never could have failed) and repoint the surviving range test at the new ceiling.
- [ ] **Step 4: Run the touched classes** — `gradle --no-daemon --console=plain test --tests
...
- [ ] **Step 5: Run the structural net + a scoped wiring IT** (AC-7) —
...
- [ ] **Step 6: Generalization-audit pass** — search for any remaining place a per-module constant
      states a platform-wide bound. Append to the log.
- [ ] **Step 7: Commit** — `git commit -m "refactor(#456): read both pools' drain ceilings from the
...
- [ ] **Step 1:** Run `riviera-docs-freshness` over `origin/main...HEAD`, including the **counting
...
- [ ] **Step 2:** Patch `CLAUDE.md`'s `notification` module row where it states the drain derivation,
      and `docs/runbooks/observability.md` if it names the mail budget as the platform's.
- [ ] **Step 3:** Finalize Execution status **in this PR's own last commit**, citing `merged via PR #NN`
      (never a merge SHA — the squash SHA cannot exist before the merge; three consecutive slices paid
      that tax).
- [ ] **Step 4:** Run the Self-review checklist below; leave nothing unchecked without an Open Question.
...
- [ ] **AC-1:** Run `gradle test --tests "*ShutdownDrainArchitectureTest*"` → `everyDrainingPoolIsAccountedFor` PASS. Verified at commit `<sha>`.
- [ ] **AC-2:** Same run → `theDetectorFindsAnOversizedFixturePool` PASS. Verified at commit `<sha>`.
- [ ] **AC-3:** Run `gradle test --tests "*ShutdownBudgetTest*" --tests "*ShutdownDrainArchitectureTest*"` → both budget assertions PASS, and the oversized claim set returns `false`. Verified at commit `<sha>`.
- [ ] **AC-4:** Same run → `everyDiscoveredPoolDeclaresAClaim` PASS. Verified at commit `<sha>`.
- [ ] **AC-5:** Run `gradle test --tests "*MailTransportPropertiesTest*"` → PASS. Verified at commit `<sha>`.
- [ ] **AC-6:** Run `gradle test --tests "*RefundExecutorPropertiesTest*"` → PASS. Verified at commit `<sha>`.
- [ ] **AC-7:** Run `gradle test --tests "*RefundExecutorWiringIT*"` → PASS (or documented Docker skip; CI owns the authoritative run). Verified at commit `<sha>`.
...
- [ ] **Availability** section filled (justified `N/A`); no SQL added, and the global-query-timeout ban is untouched (invariant #2).
- [ ] Pool + cutoff rules honored (invariants #3, #4) — untouched by this slice.
- [ ] **Modulith** section filled; `shared` stays free of business logic and of any module dependency; no new `allowedDependencies` grant needed (invariant #11).
- [ ] **Payment/payout** section filled; no money moves; the refund pool still initializes with its shipped window (invariants #5, #8, #9).
- [ ] Refund policy enforced server-side (invariant #10) — untouched.
- [ ] Timezone correct (invariant #6) — no time-of-day logic added; the constants are durations, not instants.
- [ ] Booking codes unguessable (invariant #7) — no new log line carries one; the guard logs nothing.
