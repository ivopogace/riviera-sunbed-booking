...
comment instead of migrating into the body) · `riviera-modulith` (**added at the review gate, F-4** —
the routing table fires it on *any* backend Java create/modify, and the plan had reasoned "no
boundary changed" instead of loading it. Re-vetted after loading: `ExpireRequests` is an internal
use-case port in `booking/application/request/`, the one module sliced by use-case; javadoc-only, no
surface, placement or dependency change — the conclusion held, the gate step had been skipped) · `playwright-cli` (the computed-style drift check
...
| R-4 | A directive host `[class]` binding **replaces** a static `class` on the same element, silently dropping `.chip`/`.chip--*` — which (the plan assumed) four spec files and the e2e query | med | high | Was: follow the `amenity-chip.ts` precedent — the directive owns the whole class list including the markers, and the consuming template carries no `class` attribute | claude | **closed — the risk was false in BOTH halves** (F-1, F-2). The clobber does not exist: in Angular 22 a static class and a host `[class]` **merge**, measured at the review gate and now pinned by `status-chip.spec.ts`; the `amenity-chip.ts` comment that asserted it has been corrected. And nothing queried `.chip` in the DOM anyway — both e2e suites select by `data-testid`, and `booking-status.spec.ts` asserts the modifier as *data*. The mitigation is kept on its own merit (single-sourcing the vocabulary), not as protection against a hazard that never existed |
...
|---|---|---|---|
| F-1 | self, prompted by the user's "use the angular mcp search doc" | `status-chip.ts`'s TSDoc claimed a host `[class]` binding **replaces** a static `class` on the element — inherited from `amenity-chip.ts`'s comment. The Angular docs do not state this either way, so I measured it: in Angular 22 they **merge**. The false claim had already propagated once (into my own file), which is the argument for fixing the source | fixed — both comments corrected, the design kept (emitting markers from the directive is an *ownership* choice, not a clobber workaround), and the real behaviour pinned by a new `status-chip.spec.ts` case so a future framework change fails loudly |
| F-2 | review (reviewers 1, 3 **and** 4, independently) | `booking-view.contrast.spec.ts` still cited `booking-view.scss` (deleted by this PR) and "the shared `status-chip` **mixin**" (retired by this PR) — the exact #472/#473 failure class. Worse: R-6 named that file as a known site and claimed it re-pointed | fixed — citations now point at `booking-view.ts`'s `CLS` recipes and `shared/status-chip.ts`; R-6 corrected to record the false claim rather than quietly closing it |
| F-3 | review (reviewer 4) | My *own* phase-4 re-point was misdirected: `operator-console.contrast.spec.ts`'s "(see `_glass.scss`)" was a generic pointer to the **css:S7924 technique**, and I mechanically redirected it to `shared/status-chip.ts`, which the operator console does not consume | fixed — the file pointer is dropped entirely and the technique named inline; the next sentence already says the values mirror `operator-console.scss` |
| F-4 | review (reviewer 1, RV-PROC-1) | `riviera-modulith` missing from *Skills consulted* though the diff edits a backend Java file; the routing table fires it on any backend Java create/modify | fixed — skill loaded, `ExpireRequests` re-vetted through it (conclusion unchanged), line updated |
| F-5 | review (reviewers 1 and 4, RV-STYLE-1) | Five new multi-line `//` inline comments (`status-chip.spec.ts` ×2, `booking-view.spec.ts`, `booking-view.ts`, `find-booking.scss`) — the same class PR #476's review finding 7 flagged | fixed — each trimmed to one line |
| F-7 | review (reviewer 5) | `status-chip.ts`'s TSDoc claimed the markers are "retained as inert test hooks — the unit specs and both e2e suites query them". **False, and copied from `amenity-chip.ts` where it is true**: nothing queried `.chip` in the DOM before this PR — both e2e suites select by `data-testid`, and `booking-status.spec.ts` asserts the modifier as *data* out of `STATUS_META`. Second false claim inherited from the same sibling comment (see F-1) | fixed — the TSDoc now states the real reason (single-sourcing the `STATUS_META` vocabulary) and says outright that no pre-existing test queried them, so `status-chip.spec.ts` is what makes them a hook rather than a claim |
| F-8 | review (reviewer 5) | `BookingView`'s TSDoc justified the `CLS` map with "most appear three to seven times" — but 16 of 31 entries are used **once**; the counts describe the shared *base* constants, not the map's entries | fixed — the doc now says the bases are what is single-sourced and that many entries are used once, deliberately, to name a variant beside its siblings |
| F-9 | review (reviewer 5, adjacent pre-existing) | `RequestReleaseService#withdraw`'s javadoc said withdraw is "disjoint from both by the same `status` predicate" — incoherent (sharing a predicate is the opposite of disjoint) and contradicting the class javadoc 30 lines above. This PR's new `ExpireRequests` text sends readers to exactly that class, so the contradiction is now load-bearing | fixed — the method javadoc now says withdraw is guarded on `status` alone like decline, that this is the opposite of disjoint, and that the row lock picks the winner. Out of #477's literal scope, taken because this PR made that paragraph the canonical reference |
| F-6 | review (reviewer 1, RV-STYLE-1 borderline) | Multi-line `/** */` doc comments on the module-private consts `CLS` and `FILLS`, which are not among the exempt surfaces (component / service / directive / exported type) | fixed the way the rule prescribes — the rationale moved **onto** the exempt surfaces (`BookingView`'s and `StatusChip`'s TSDoc), leaving a one-line doc on each const |
