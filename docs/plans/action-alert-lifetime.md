...
| 5 — Close-out: docs-freshness F-11, plan-doc final state | ✅ | `06c7fdc2` |
| 6 — Final pre-merge review gate: F-12, F-13 | ✅ | this commit |
...
- **Review gate, second run (pre-merge, at the maintainer's request).** Re-run over the *current*
  diff, because the first pass reviewed `7d355be` and two commits had landed since — the re-review
  `pr-gates.md` §1 step 3 requires. Full five-agent fan-out plus the overlay. Three came back clean
  (bug scan; git-history; prior-PR-comments, which independently re-ran the `role="alert"`
  enumeration and confirmed the "last two ungated" claim). Two findings: **F-12** and **F-13**, the
  latter accepted in part and declined in part on measured evidence.
...
| F-12 | Final pre-merge review gate (comment-compliance agent) | `operator-password.ts:30`'s one-liner sat above **both** `cls.notice` and `cls.submitError` and read as covering the pair — but the same diff `@if`-gated `submitError`, so "keeps its resting margin while silent" is false for it: that element no longer exists while silent. Only the notice is always mounted | fixed — reworded to lead with "Always mounted", the property that distinguishes the two, so it cannot be read as covering the gated one |
| F-13 | Final pre-merge review gate (CLAUDE.md-compliance agent), **partially accepted** | `revealOutcome()`'s TSDoc carried decision archaeology — `(#828)`, "a failed change focused a blank paragraph", "the `:not(:empty)` that was meant to prevent exactly that" — which `frontend/.claude/CLAUDE.md` §Comments and `riviera-java-conventions` §6d forbid in TSDoc ("the contract, not the changelog… no issue numbers, no decision history"). Fixing RV-STYLE-1's one-line rule by moving prose *into* TSDoc is exactly the move §6d bounds | **accepted for the production method TSDoc, declined for the three test-file headers.** `revealOutcome()`'s doc trimmed to the contract plus the warning that keeps it from collapsing back into one selector list, history relocated here per §6d's "relocate, don't delete". The agent also flagged the `a11y.spec.ts` and two `e2e.ts` headers; **declined on measured evidence** — §6d is scoped to Javadoc on a documented surface ("every `api/` port depends on it", "what a caller must know"), and an issue number in a *test* header is established convention on `main`: 11 of 65 `frontend/e2e` file headers cite one (`#605`, `#724`, `#744`, `#749`, `#761`, `#765`…), several with the same supersedes/regression framing. In a test the issue number **is** contract — it names the regression the test guards |
