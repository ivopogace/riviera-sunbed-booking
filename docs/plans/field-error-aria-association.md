...
- **OQ-1 — deferred to a human, with an issue.** Does keeping `role="alert"` alongside the
  ...
  slice does **not** claim the question is answered. **Answered 2026-08-31, via #824's NVDA
  pass on Windows (deployed build):** no — Pass A (review form, `submitAttempted()` shape)
  announced once on appearance and reads the message once as the field's description on
  re-focus, and Pass B (pricing tab site 16, the F-7 coincidence — error appearing while its
  own field held focus) also announced once. Both techniques stay as shipped; no follow-up
  slice. #824 closed with the results.
