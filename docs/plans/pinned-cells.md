...
| F-5 | Sonar (PR #1046 at `18587616`: gate passed, `java:S1192` critical smell — `"NO_SUCH_VENUE"` duplicated three times in `VenueAdminController`, the third being the beach-map read; 96.35% new-code coverage, 0 duplicated blocks, 0 bugs/vulnerabilities over 674 new lines) | the literal becomes `NO_SUCH_VENUE_CODE` beside its detail constant | fixed in the Sonar-fix commit; the list re-read after its analysis |
