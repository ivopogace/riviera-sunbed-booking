...
- [ ] **AC-6:** Given an authenticated operator who does not own the path `venueId`, when a
  ...
  or write (invariant #13). *Pinned by:* `CrossVenueDenialIT.rowRenameByNonOwnerIs403` +
  `VenueAdminServiceTest.renameByANonOwnerIsDeniedBeforeAnyWrite`.
...
| R-3 | Probe-then-write race: `addSet` does not take the venue row lock, so it can insert a set carrying the target label between the duplicate probe and the `UPDATE`, and `set_position_cell_uniq` then surfaces as a `500` rather than the honest `409`. | low | low | **Accepted, mirroring the existing posture** — `addSet` itself probes with `findConflict` then relies on the same constraint as "the hard backstop" (`EditBeachMap` javadoc). Both writers are the same operator's console on the same venue; the window is one statement wide. If it ever bites, the fix is to map `DuplicateKeyException` to `ROW_NAME_TAKEN` in the adapter — noted here so a future session doesn't re-derive it. | plan author | **accepted, with its stated mitigation corrected by review finding F-3**: when the position numbers do not collide the UNIQUE constraint does not fire at all, so the failure is a silent duplicate label rather than a 500. F-3's fix for that was reverted (see the findings register), so the residual stands as written — narrowed by the fact that both writers are the same operator's console and the window is one statement wide. |
...
|-------|--------|---------|
| 0 — Backend inner hexagon (command, outcome, port, service, adapter) | ✅ | `b388486` |
| 1 — REST edge + integration tests | ✅ | `65081c0` |
| 2 — Layout-editor per-row rename (Angular) | ✅ | `84d0238` |
...
| F-3 | review (`/code-review`, high) | **The one-label-per-row invariant is enforced only on the rename path.** `ROW_NAME_TAKEN` refuses a duplicate label, but `LayoutCommand.duplicateWithin` checks only `(rowLabel, positionNo)` and `(gridX, gridY)`, and `findConflict` the same two — so `replaceLayout`, `addSet` and `editSet` can all still create two grid rows sharing a label, no DB constraint backs it, and none of those three takes the venue-row lock (so `addSet` can also slip a duplicate in between the rename's label read and its UPDATE, and a stale `editSet` can split a renamed row by rewriting one set's `row_label`). Verified: `updateSet` does write `row_label`; `duplicateWithin` does not check labels. This supersedes and corrects R-3, whose stated mitigation ("the UNIQUE constraint is the backstop") is wrong whenever the position numbers do not collide — the merge is then silent, not a 500. | **built (`1b9fba1`), then REVERTED — maintainer's call after the re-review.** Enforcing the rule on all four paths introduced two regressions the re-review caught and I verified in the code: (1) `placementAt` derives a new set's label from the grid letter when the target grid row is empty, so once another row had been renamed onto that letter, the set editor — which has no row-label field — could never place a set on that row again; (2) `findConflict`'s check fired on **every** `editSet`, including edits leaving `rowLabel`/`gridY` untouched, so any venue already carrying a duplicate label (the API permitted it until now) became permanently uneditable, with no normalizing migration. **Root cause — a planning error, not a coding one:** the rename's own `ROW_NAME_TAKEN` is a property of one endpoint (*this write will not create a mess*); F-3 promoted it to a system-wide invariant (*nothing may ever create this mess*) across three paths built assuming labels are free-form, plus existing data. That is a feature needing a migration and a UI affordance, not a review fix, and it should have been pushed back on when the review raised it rather than offered as a fix-here option. The rename keeps its own refusal, which remains in scope and correct. The wider gap stays documented in `RESPONSIBILITIES.md` §`venue`; **no follow-up ticket was opened** — the maintainer's standing instruction is to ask first. |
...
| F-5 | review | The rename re-entrancy guard checked only `renamingRow`, not `saving()`, and the button stayed live during a bulk save — so both writes could be in flight off the same `expectedVersion`. | fixed — guard now covers `saving()`, `[appBusy]` covers it too; pinned by "does not race a rename against an in-flight bulk save" |
| F-6 | review | `seedFrom` / `reloadAfterStale` / `onSetsChanged` re-index the rows but left `renamedRow` / `rowNameError` pinned to the old index, so a notice could describe a different row. | fixed — `clearRenameNotices()` at the top of `seedFrom`, above its early return; pinned by "clears a rename error when a stale reload re-indexes the rows" |
| F-7 | review | The `ROW_NAME_TAKEN` probe ran before the source row's existence was established, so renaming a row that no longer exists onto a taken label answered 409 instead of 404. | fixed — `distinctRowLabels` answers both questions off one read, existence first; pinned by `renameOfAMissingRowOntoATakenLabelIsNotFoundNotTaken` |
| F-8 | review | `newLabel` was never trimmed, so `" A"` slipped past the exact-match duplicate refusal and stored a label that renders identically to `"A"`. `SetCommand` had the same gap, so the bulk path could seed it. | fixed — `VenueFieldValidation.strip` applied in **both** commands (generalization pass below); case deliberately left alone, since `"back row"` and `"Back row"` read differently |
| F-9 | review | `VenueSetWriteConcurrencyIT`'s javadoc claimed "the set-writes share ONE token" but only raced replace-vs-reprice. | fixed — added `replaceAndRenameCannotBothWin` (6 repetitions), which also asserts the venue ends with one distinct label |
...
- `platform/src/main/java/ai/riviera/platform/venue/application/SetCommand.java` — review F-8: strip the row label on the bulk path too.
- `platform/src/main/java/ai/riviera/platform/venue/application/RowPriceCommand.java` — re-review R2-8: the third member of the strip population, missed by the first audit.
- `platform/src/main/java/ai/riviera/platform/venue/application/VenueFieldValidation.java` — review F-8: the shared `strip` helper.
...
- `frontend/src/app/operator/console-venue-map.ts` — review F-11: the rename joins the `reset()` call-site list.
...
|---|---|---|---|---|---|
| 2026-08-20 | Phase 5 — review F-8 (untrimmed label) | Every command that writes `set_position.row_label` (mechanism: its compact constructor calls `VenueFieldValidation.requireText` on a `rowLabel`/`newLabel`) — each judged on whether padding can reach storage | `grep -rn "rowLabel" platform/src/main/java --include=*Command.java` | **3** — `RowNameCommand`, `SetCommand`, `RowPriceCommand` | All three strip now. **The first pass of this audit recorded only two and was wrong** — it enumerated the commands that *write* a label and missed `RowPriceCommand`, which *addresses a row by* one, so an un-stripped path label silently missed its row. The re-review caught it (R2-8). This is #641's mechanism-vs-resemblance failure repeated in a slice whose own plan cites #641: the recorded command did return three, and the miss was in reading it, not running it. |
