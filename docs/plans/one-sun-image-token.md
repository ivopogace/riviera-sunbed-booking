...
`origin/main...HEAD` at close-out — 8 findings, all patched; see the close-out note) · `prototype` (built the spike
...
- [x] **AC-1:** Given `src/tailwind.css`, when the stylesheet is read as text, then
      ...
      theme block can override it. _Seam:_ `src/tailwind.css` as text via
      `testing/stylesheet-tokens.ts` (`declarationsOf`, `baseBlock`) · _Pinned by:_
...
- [x] **AC-2:** Given every non-spec `.ts`/`.html` under `src/app`, when swept for a radial
      ...
      reports the paths to fix, not the sources. _Seam:_ the app-source sweep over `src/app`
      (`class-o-tint-tokens.contrast.spec.ts`'s `appSources()` pattern) · _Pinned by:_
...
- [x] **AC-3:** Given the brand mark, a photo-less venue card and a photo-less venue-map band
      ...
      `getComputedStyle`, then all three resolve to the same value. _Seam:_ the rendered DOM at `/`
      and `/venues/1` (mocked API) · _Pinned by:_ `e2e/sun-token.e2e.ts` › `all three suns resolve
...
- [x] **AC-4:** Given the home venue card's `.photo-sun` in a real render, when its computed
      ...
      opacity is `1` — the sun covers the cyan instead of compositing green against it. _Seam:_ the
      rendered `.photo-sun` at `/` · _Pinned by:_ `e2e/sun-token.e2e.ts` › `the card sun is opaque,
...
- [x] **AC-5:** Given the venue-map band's sun, when its computed `background-image` is compared
      ...
      pixel at the consumer whose values were adopted. _Seam:_ the rendered
      `[data-testid="map-banner-empty"]` at `/venues/1` · _Pinned by:_ `e2e/sun-token.e2e.ts` › `the
...
- [x] **AC-6:** Given a photo-less venue card, when the location text's contrast floor is
      ...
      white-photo worst case (`#ffffff`) still bounds the sun's brightest stop (`#fff6da`). _Seam:_
      the composited-contrast maths in `src/testing/contrast.ts` · _Pinned by:_
...
| Map band sun: spec asserts the fill carries no `rgba` stop (#704)         | **changed**   | the assertion moves to the token guard (AC-2/AC-4), where it now covers _all three_ consumers instead of one; `venue-map.spec.ts` keeps a consumes-the-token assertion in its place |
...
- **Open question:** Are the app shell's sun and the home one meant to be the same sun?
  — _Resolved:_ the question was mis-framed; there are **three** suns and they split 1/2 by
...
- **Open question:** May the PR be opened? — _Resolved:_ the maintainer authorised it and asked
  for the flow driven to merge. Opened as **PR #885**, ready for review.
- **Open question:** May `spike/882-sun-merge` be pushed? — _Resolved:_ not authorised, so not
...
- **Open question:** Which paint does the merged token carry? — _Resolved:_ the map's opaque
...
- [x] **The review gate ran in full** — per the invocation ladder in riviera-sdlc `references/pr-gates.md` §1 _plus_ `riviera-review-overlay`.
