...
| 5 — Docs correction (`bugfix/286-correct-peer-trust-model`) | ✅ | PR **#288**, merged `f4863f8`. Removed the falsified "unset it" guidance from `main`; CI 10/10 green, Sonar issue list `total:0` |
| 6 — Merge close-out (`pr-gates.md` §3) | ✅ | `riviera-docs-freshness` over `f213d82..f4863f8` found **1** finding — `production-hardening.md` still carried the falsified "the trust list never has to enumerate the CDN's ranges" sentence, missed by #288 — patched in PR **#289**. `CLAUDE.md` / `CONTEXT.md` / `RESPONSIBILITIES.md` / `docs/agents/` state nothing this span contradicts (their only rate-limit mention is the #115 registration bucket). `graphify update .` run |
