...
| `booking-view.ts:89,99` (banner body ink ×2) | `#334a52` | shares one class string with the strong ink at `:89`; maintainer-approved scope call, 2026-09-02 | `--riv-banner-body-ink` |
...
|---|---|---|---|
| F-5 | review gate — in-code comment audit | Three factual errors in the ledger prose this PR wrote: the class-T note said **five** calendar sites where the arithmetic needs four (4 + 1 + 3 = 8, the paragraph's own headline), the banner row pointed at `booking-view.ts:98` when this PR's own reformat pushed `confirmQOnBanner` to `:99`, and the weekday-header `.72` was filed under #852's beside-a-literal check, which its `<th>` does not trigger. Major — a ledger that miscounts its own population is the failure mode this whole slice exists to correct | fixed — all three corrected; the `.72` now carries its real cause (the contrast spec's `CHROME_INKS` set), which the plan doc had right all along |
| F-2 | review gate — recurring finding across PRs #862, #871, #875, #878, #883, #885 (§6d "Javadoc states the contract, not the changelog") | The re-cut's narrative — the refusal, the scope movements, the repaint — was written out in full in **four** places: `tailwind.css`'s three declaration blocks, `glass-tokens.ts`'s mirrors, and both spec headers, with per-test doc comments restating it again. None carried a terse `Rationale:` pointer, though `colour-literal-token-audit.md` in this same PR already holds the narrative at length. This is the class reviewers trimmed in five of the eight most recent PRs in this series. Major | fixed — every site cut to contract + `Rationale: docs/design/colour-literal-token-audit.md`; the ledger stays the one narrative home |
| F-3 | review gate — RV-FE-7 / `riviera-tailwind` "take a per-state class ternary whole" | `availability-calendar.html:20` now reads `text-[#0a3f4e] … aria-disabled:text-riv-calendar-ink-disabled` — a token beside a literal in one expression. **Not upheld as a violation:** two review agents split on it, and the narrower reading is right — `#0a3f4e` is the calendar's ACCENT (every `CALENDAR_TINTS.ring` and `CALENDAR_BAR.fill`), a different family with its own surfaces, fenced as a Non-goal. #852's rule is about a literal *of the token's own value* | not-a-defect, but made explicit: a new assertion pins that the accent stays literal and that its value is the ring/bar family's, so the omission reads as a decision rather than a gap |
| F-4 | review gate — bug scan | The e2e's `UTILITIES` sweep omits three tokens without saying why, and one shortened comment read as contradicting the selector beneath it | fixed — the omission is now explained (all three are consumed through variants, so Tailwind emits compound rather than bare-class selectors, and each is proven on its rendered box instead); the comment reworded |
