...
|---|---|---|
| `success` glyph fill = `--riv-accent-chip-fill`, a teal tint composited over the themed card | **changed** | now the opaque `--riv-medallion-positive-fill` (`#d9f2f7`). Light themes shift subtly (composite `#c5ee f5`–`#d9f2f7` → flat `#d9f2f7`); **dark changes visibly**: a dark teal circle becomes a pale ice-blue one |
...
- **Assumption:** the six artboard lines are the complete diverged set (`riviera-sign-in.dc.html`
  127/136/145, `riviera-sunbeds-liquid-glass-v3.dc.html` 538/642/676). Enumerated by grepping the
...
- [ ] **Step 1: Write the failing tests** — the `MIGRATED_SITES` row for
...
- [ ] **Step 2: Run them, verify they fail** — on a pre-phase-0 tree they would; on this tree the
...
- [ ] **Step 3–4: Green** — `npm test -- fixed-fill-token-skins` and
      `npm run test:e2e:a11y -- fixed-fill-state-skins` → PASS

- [ ] **Step 5: Commit** — `git commit -m "Guard the converged outcome-card medallion in both themes (#869)"`
...
- [ ] **Step 1:** F-5 row → `done — #869, PR #NN`, carrying the verdict, the artboard finding,
      and the class-O correction.
- [ ] **Step 2:** the six `<!-- as-built diverges — see #869 -->` pointers.
- [ ] **Step 3:** the `--riv-medallion-*` declaration note in `tailwind.css` and the population
      count in `glass-tokens.ts` (five medallion sites → seven).
- [ ] **Step 4:** `node scripts/check-plan-file-structure.mjs --diff origin/main` → clean.
- [ ] **Step 5: Commit** — `git commit -m "Record the F-5 verdict in the ledger and at the artboards (#869)"`
...
- [ ] **AC-1:** `npm test -- auth-page.contrast` → PASS, all three themes.
- [ ] **AC-2:** `npm run test:e2e:a11y -- fixed-fill-state-skins` → PASS, both themes.
- [ ] **AC-3:** `npm test -- fixed-fill-token-skins` → PASS.
- [ ] **AC-4:** read the four substrate files in the PR diff.
