...
| F-7 | CI (`SonarCloud scan` job on `4afc9fc`) | The scan step failed **7 seconds** in on a markdown-only commit, while the same job had just succeeded on `28e704b` — a transient upload/API failure, not an analysis result (a real scan takes minutes, and Sonar does not gate on markdown). The API token cannot re-run jobs, so it was re-triggered by the next push | re-triggered; outcome recorded on the PR |
