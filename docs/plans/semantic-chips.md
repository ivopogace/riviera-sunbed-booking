...
"New" chip is a _trust_ signal, not a booking-mechanics one, and that the four chips all sit
...
- [x] **AC-1:** Given the shared semantic-chip recipe, when its ink and fill are read, then
      ...
      _Pinned by:_ `semantic-chip.contrast.spec.ts` → "the semantic-chip ink meets AA on its solid fill".
- [x] **AC-2:** Given the semantic-chip fill, when it is compared with both descriptive-chip
      ...
      _Pinned by:_ `semantic-chip.contrast.spec.ts` → "the semantic fill is a different family from the $name fill".
- [x] **AC-3:** Given a Discover card and a beach-map header for the same venue, when both
      ...
      amenity/to-water chips do not. _Pinned by:_ `home.spec.ts` and `venue-map.spec.ts`.
- [x] **AC-4:** Given the Discover mode chip over an arbitrary cover photo, when its
      ...
      strictly stronger than the 0.85 glass backing it replaces. _Pinned by:_
...
- [x] **AC-5:** Given both themes in a real browser, when Discover and the beach-map header
      ...
      on both surfaces, and axe reports no serious violations. _Pinned by:_ `discovery-flow.e2e.ts`.
- [x] **AC-6:** Given the four call sites, when the directive is applied, then each chip's
      ...
      the box metrics that decide layout are not touched. _Pinned by:_ review of the diff plus
...
| R-4 | On the dark map-header glass a dark fill reads as _receding_ next to the pale amenity pills, inverting the hierarchy | med        | med    | the pill takes a **lighter** accent rim (`#2f7d92`) so its shape reads as a solid object on the dark panel; AC-2 states the family separation as a ratio against both descriptive fills                          | claude | closed — 6.4:1 vs the neutral fill, 6.0:1 vs the to-water fill                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **Assumption:** "designer's choice within the Liquid Glass token system" in the issue
  delegates the _choice of accent_ to the implementer, so no `AskUserQuestion` is due; the
  choice made is the inverted accent pill described in **Architecture**, and the fill is an
  existing system colour (`--riv-cta-grad`'s dark stop), not a new one. — _Owner:_ claude ·
  _Resolved:_ proceeded on the delegation; the chosen accent is `--riv-cta-grad`'s dark stop, so no new token was added. The maintainer sees it rendered at the review gate and can redirect the accent without touching any call site — the recipe lives in one directive.
- **Assumption:** grouping the "New" chip with the mode chip is deliberate on the issue's
  part even though "New" describes _trust_, not _booking mechanics_; both are read as
  platform-authored claims rather than venue-authored descriptions, which is the line the
  treatment actually draws. — _Owner:_ claude · _Resolved:_ built as the issue specifies; the reframing is recorded in `semantic-chip.ts`'s contract so a later reader knows which line the family split draws.

## Availability & concurrency (invariant #2)

N/A — presentation only. The slice writes no `availability(set_id, booking_date)` row, reads
no booking state, and changes no call into `AvailabilityClaim`. The mode chip _reports_ a
...
slice, and again on the fix rounds per the re-entry rule. Every finding is in the register below
(F-* the slice, then G-_, H-_, J-_, K-_); the register is the record, so this paragraph states no
total — each round's own fix commit is, necessarily, the one round nothing has re-reviewed.

**Where the loop was stopped, and why.** Round counts went 4 → 6 → 4 → 6 → 6 without converging, so
this is recorded rather than pursued further. The character of the findings is the reason it is safe
to stop: **every round from the first confirmed the executable change was sound** — no correctness
defect was ever found in the shipped styling, and the reviewer said so explicitly each time. What
kept regenerating was prose _about_ the tests, which is structurally self-generating: each round's
fixes need describing, the description is then a round behind, and a description is itself
reviewable surface. The substantive findings across rounds 2–5 (the mirror's ties, the two specs'
set-equality, the amenity filter's `bg-` prefix) are all fixed; what remains flagged is redundancy
between assertions that subsume one another, and prose that could be tightened further. That is a
judgement call to leave with a human, not a defect to keep pushing at.
...
what is _checkable at the time it is written_ (a precondition rather than a past-tense claim) and
to keep the enumeration in one register rather than restated in prose.

**Merge precondition.** This is the PR's last commit, so the gates run on _this_ head: the merge is
...
| 5 — re-review rounds 2–4 (G-_, H-_, J-*)                      | ✅     | `e04bfe6`, `84bf867`, final commit |
...
| G-2 | review gate (round 2)              | four places still said the `home.contrast.spec.ts` proof was "repointed", contradicting F-4's deletion in the same commit; R-1 was marked closed by the very outcome it predicted                                  | fixed — ledger, risk row, file structure and phase step all corrected; R-1 now reads _materialised, then closed_                                                                       |
...
| H-4 | review gate (round 3)              | the `styles.scss` pointer named the _importing_ spec as the mirror; the mirror is `testing/glass-tokens.ts` — a renamer following it would have reproduced F-2                                                     | fixed                                                                                                                                                                                  |
...
| K-1 | review gate (round 5)              | `amenity-chip.spec.ts` filtered `bg-[` where its sibling filtered `bg-`, so a `bg-white/60` beside the opaque fill would have escaped only on the descriptive side                                                 | fixed — both filters are now `bg-`, and the comment says why the prefix matters                                                                                                        |
| K-2 | review gate (round 5)              | `chip-fills.ts` and a spec comment credited the whole-list pin with a guarantee the test below it already made on its own                                                                                          | fixed — each assertion's comment now names only what that assertion adds                                                                                                               |
| K-3 | review gate (round 5)              | two File-structure bullets still described the pre-H-2 versions of the two chip specs                                                                                                                              | fixed                                                                                                                                                                                  |
| K-4 | review gate (round 5)              | `discovery-flow.e2e.ts` holds a third copy of the pair as `rgb()` triples, so "the ONE test-side mirror" was false                                                                                                 | fixed — scoped to the unit suite, with the e2e copy named as the deliberate exception (black-box by design, and self-policing: it is compared against the real page)                   |
| K-5 | review gate (round 5)              | "the gate ran again on **each** fix round" is written inside a commit no round has reviewed                                                                                                                        | fixed — the record now says so outright rather than implying otherwise                                                                                                                 |
| K-6 | review gate (round 5)              | under the whole-list pin, the class-level halves of the remaining semantic-chip tests are subsumed                                                                                                                 | acknowledged, not removed — each kept test still carries a distinct _why_ (opacity, geometry, the two mirror ties); recorded at the loop stop above as judgement to leave with a human |
...
- `frontend/src/app/shared/semantic-chip.spec.ts` — pins the whole recipe as a set, plus the opaque-fill and mirror-tie proofs
...
- `frontend/src/app/shared/amenity-chip.spec.ts` — ties the mirror to what the directive renders, as a set (G-3, strengthened by H-2/K-1)
