...
| F-5 | angular-cli MCP verification (`search_documentation`, v22) | **`afterNextRender` was called with a bare callback**, which Angular runs in the `mixedReadWrite` phase — the one its own docs say to *"never use if it is possible to divide the work among the other phases"*, warning it *"may worsen application performance by causing additional DOM reflows"*. Here it divides exactly: locating the element is a read, focusing it is a write. | fixed-in-`e94ea96` (split into `earlyRead`/`write`). **Pre-existing repo-wide**: 15 files call `afterNextRender` and the sampled ones all use the bare form, so this PR stops the spread rather than clearing the debt — a sweep is its own change, flagged to the maintainer, not slipped in here. |
