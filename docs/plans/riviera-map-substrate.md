...
- [ ] **AC-1:** Given the map resources at the configured `file:` location, when a client GETs
...
- [ ] **AC-2:** Given the shipped `platform/map/style.json`, when its `sources.*.url`,
...
- [ ] **AC-3:** Given `window.__RIVIERA_FAKE_MAP__` is unset, when `appConfig`'s `MapEngine`
...
- [ ] **AC-6:** Given the Discover page below the `lg` breakpoint, when it renders, then the list
...
- [ ] **AC-7:** Given the mocked suite with the **real** engine and `/map/**` fulfilled from
  ...
  *Pinned by:* `discover-map.e2e.ts` › `the list renders and works before the map asks for
  anything`
- [ ] **AC-8:** Given the same real-engine setup, when the map has loaded and been panned and
...
- [ ] **AC-9:** Given the fake engine on a phone viewport, when the map is opened, then
...
- [ ] **AC-10:** Given `/legal/privacy`, when it renders, then a section names the self-hosted map,
...
- [ ] **AC-11:** Given the repo, then `docs/adr/ADR-0022-self-hosted-map-resources.md` exists with
...
| R-8 | OSM Liberty's raster hillshade source and MapTiler tile URL survive the rewrite | med | high | The script deletes the raster source and its layer and rewrites the vector source to `pmtiles:///map/riviera.pmtiles`; AC-2 fails on any host | slice | — |
| R-9 | Glyph coverage: only ranges 0–1279 ship per font stack; a label with a character outside them renders blank | low | low | Riviera labels are Latin/Latin-Extended (Albanian ë/ç) with Greek as a minority; the runbook names the ranges and how to widen | slice | — |
...
- **Assumption:** the OSM Liberty style (BSD-3, `maputnik/osm-liberty` gh-pages), its sprites,
  and the Roboto glyph ranges from `orangemug/font-glyphs` (Apache-2.0 fonts) are acceptable
  licences for this deployment; attribution to OpenStreetMap is the only runtime obligation
  (ODbL). — *Owner:* maintainer · *Resolves by:* review.
- **Assumption:** Planetiler 0.10.2 (`java -jar planetiler.jar --download --area=albania
  --bounds=… --output=….pmtiles`) produces an OpenMapTiles-schema archive the OSM Liberty layers
  read; the flags are from Planetiler's README/PLANET.md, not from a run here (R-1). — *Owner:*
  runbook run · *Resolves by:* the first regeneration.
...
- [ ] **AC-1 … AC-11:** see the phase commits; filled at close-out.
