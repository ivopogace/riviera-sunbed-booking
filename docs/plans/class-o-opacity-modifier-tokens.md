...
| F-2 | CI (`6ecfcc4`) | My own `e2e/class-o-tint-tokens.e2e.ts` went red on both paint tests. It pinned the armed tool's fill as the literal string `oklab(0.723426 -0.0974235 -0.0681883 / 0.2)`, captured from this sandbox's Chromium; CI's build serializes the same colour as `oklab(0.723439 -0.0974177 -0.068208 / 0.2)`. Same paint, different float precision — and my local run passed because it used the binary that produced the snapshot. **Fixed by removing the snapshot, not by adding a tolerance:** the test now renders the outgoing literal (`color-mix(in oklab, #2bb8d4 20%, transparent)`) as a probe in the same page and compares against it, which asserts the claim the slice actually makes — the token form paints what the literal painted — in whatever browser runs it. Negative control re-run: a one-digit change to `--riv-select-tint` still reddens all three tests | fixed-in-`42f1c83` |
| F-3 | review gate (agent 5 — comment truth) | `booking/solid-btn-tokens.contrast.spec.ts` — **my own regression, introduced by the phase-4 docs-freshness sweep.** I tightened a vague-but-true "Three remain" into a specific falsehood: "Two remain as **LITERALS** … the two class-F medallions (`shared/failure-panel`, `booking/booking-pay`)". Both consume `#a3372a` through `--riv-medallion-negative-*` and paint no literal at all (verified by grep). Rewritten to state what is true — since #864, #858 and #852, *none* of that value's homes is a literal; only the notation moved, which is why the sweep stays scoped to the family's three components rather than becoming a tree-wide value sweep | fixed-in-`1ca92bf` |
| F-4 | review gate (agents 2 + 5, independently) | This plan claimed `confirm-panel.spec.ts`'s pin was "widened to the third position (`text-[#7a4a08]`)". It was not — the spec pinned only the fill and border, and its own retained comment said the inks were deliberately left to the contrast specs. Agent 5 also established the real gap behind the discrepancy: **nothing guarded that ink.** The class-O sweeps match the `/opacity` form and the mixing guard is per-expression, so the ink sits in its own class string where a revert to `text-[#7a4a08]` would pass every check. Fixed by adding the assertion (making the claim true and closing the gap) and correcting the comment that said otherwise; negative control confirms it reddens on a revert | fixed-in-`1ca92bf` |
| F-5 | review gate (agent 4 — prior-PR comments) | The `--riv-console-tint` declaration narrated a reversed premise ("had been held back on the premise… It does not…") over ~20 lines before reaching its `Rationale:` pointer — the exact class flagged and trimmed on **#862, #871 and #856**, against `frontend/.claude/CLAUDE.md`'s "TSDoc … states the contract, not the changelog". It also duplicated into the stylesheet an argument the ledger and this plan already make in full. Trimmed 39 → 30 lines and reshaped to the terminus those slices settled on: contract, theme-invariance, the opaque-fallback warning, proof, pointer | fixed-in-`1ca92bf` |
...
|---|---|---|---|---|---|
| 2026-09-01 | phase 2 → caught by CI | **Every spec anywhere in the tree that names a value this slice migrates** — the population a *folder-scoped* test run cannot see. Phase 2 ran `operator/` + `shared/` and pushed; CI failed on a `booking/` spec | `grep -rln '#a3372a\|#a3160e' frontend/src --include=*.spec.ts` (by value, across the whole tree — the folder scope was the bug) | 4 sibling guard specs across 3 folders (`operator/`, `shared/`, `booking/`) | Widened every later phase's run to `src/app/**/*.spec.ts`. Recorded as F-1: this is the frontend face of `riviera-local-debug`'s full-suite-only failure class — scoped runs share almost nothing, and a token's blast radius follows the VALUE, not the folder |
