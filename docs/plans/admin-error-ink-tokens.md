...
`riviera-review-overlay` loaded alongside for the RV-FE / RV-STYLE / RV-PROC bank. No
correctness findings. One convention finding (F-4) scored below the confidence bar and did
not block; the maintainer asked for it to be fixed anyway, so it **re-entered at Implement**
per the `riviera-sdlc` re-entry rule — routing gate re-run for what the fix touches
(`riviera-tailwind`, `riviera-frontend`, `riviera-local-debug` for the first `npm` call),
...
| F-4 | review gate (`/code-review:code-review` + overlay, head `916e941`) | The new doc comments cited issue numbers and decision history (`#829`, `OQ-2`, `R-3`, "deliberately", "not an oversight"), which `frontend/.claude/CLAUDE.md` asks TSDoc to avoid — "states the contract, not the changelog", canonical at `riviera-java-conventions` §6d. Scored **25** at the confidence gate (the pattern is pre-existing in `tailwind.css` and in the sibling contrast specs, reviewed clean on #782) and so did not block, but the rule is real and applies to frontend files. **Fixed on maintainer instruction** rather than filtered. Applied §6d's own remedy — *relocate, don't delete*: each comment keeps its point-of-use warning in one sentence (keep the rgba pre-composed, don't collapse the dark ink duplication, the non-text boundaries are knowingly unasserted) and gains a one-line `Rationale: docs/plans/admin-error-ink-tokens.md` pointer, since this doc already carries the full argument at R-4, OQ-2 and R-3/F-1. Nothing was lost, only moved. Six comments rewritten across `tailwind.css` (×2), `testing/glass-tokens.ts` (×2), `admin-console.contrast.spec.ts`, `admin-token-inks.e2e.ts`. The `describe()` title keeps `(#829)`: a test name is not TSDoc, and both sibling contrast specs name their issue the same way (`(WCAG AA, #170)`, `#705`). | fixed in the review-fix commit |
