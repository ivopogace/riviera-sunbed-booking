...
| F-6 | review (git-history reviewer) | `booking.events.BookingCancelled`'s Javadoc asserts *"So `payout` is the sole subscriber."* — true when written, made **false** by this slice. The file is not otherwise in the diff, which is exactly why it was easy to miss: a doc the slice invalidates without touching | fixed-in-`fa3f1f2` — names `notification` as the second subscriber and says why it changes nothing above it (read-only, off the money path) |
| F-7 | review (git-history reviewer) | `RESPONSIBILITIES.md`'s numbered spine recap (step 9) still listed two consumers of `BookingCancelled` while the same file's `notification` section, edited by this slice, described the third — the file contradicting itself | fixed-in-`fa3f1f2` |
| F-8 | review (git-history reviewer, minor) | `TransactionalMailService.isSuppressedOrFailOpen`'s Javadoc said the carve-out is "deliberately not shared with `sendBookingConfirmation`" — now two registry sends share that reasoning | fixed-in-`fa3f1f2` — both named, here and in the class-level twin |
| F-9 | review (code-comment reviewer) | `ObservabilityMetrics.MAIL_RECOVERY_FAILED`'s Javadoc still said "do not sum the **four** mail counters", and `MAIL_REGISTRY_SHED`'s enumeration of "each later mail loss" omitted the new name — the slice fixed the sibling's docstring three paragraphs away and missed these | fixed-in-`fa3f1f2` |
| F-10 | review (code-comment reviewer) | `docs/runbooks/observability.md` still called the confirmation counter "the only one of the **four** that is never retried by anything" — wrong on both the count and the uniqueness, since this slice's own new section makes the identical claim about its sibling | fixed-in-`fa3f1f2` — "one of the two", with a pointer to where they differ |
| F-11 | review (code-comment reviewer) | **Self-inflicted:** `MissingBookingFact`'s Javadoc hedged "and, once #374's listener lands, `MAIL_CANCELLATION_ABANDONED`". Written in phase 1 because the constant did not exist yet, and never upgraded in phase 3 when it did — stale in the commit that shipped it | fixed-in-`fa3f1f2` — both counters now `@link`ed as settled fact |
| F-12 | review (code-comment reviewer) | `MailListenerExecutorArchitectureTest`'s class Javadoc still framed #374 as a mail "to write", while the test method this slice added below it calls #374 "the second listener the class Javadoc predicted" — the same file in two tenses | fixed-in-`fa3f1f2` — records that #374 arrived **compliant rather than caught**, which is what a fitness function is for; #373 still outstanding |
| F-13 | review (code-comment reviewer) | `notification/package-info.java` — the module's front door — described `adapter/in` as "the `BookingConfirmed` listener" and the grants as "the `BookingConfirmed` listener's reads", both singular. Not in the diff, undercounted by it | fixed-in-`fa3f1f2` — two listeners, and notes the grants did **not** widen, because both assemble through one resolver |
