...
|---|---|---|---|
| F-1 | review (prior-PR-comments reviewer) | **RV-PROC-1:** *Skills consulted* omitted `riviera-review-overlay` and `riviera-docs-freshness`, though the diff rewrites three substrate docs and the slice goes through the review gate. The reviewer found the identical note already raised and fixed on **#427, #430, #436 and #440** — this plan doc reproduced it a fifth time | fixed-in-`1988067` — both added, with what each contributed |
| F-2 | review (prior-PR-comments reviewer) | Execution-status drift: phase 5 sat `⏳` with no commit while `1ea511b` had already completed it, and the findings register read "none yet" during the review gate. The plan doc is the state store (rule 11), so this is the one defect that misleads the *next* session | fixed-in-`1988067` — phase 5 ✅ with both commits; stage pointer now carries the CI + Sonar evidence |
| F-3 | review (prior-PR-comments reviewer) | `CLAUDE.md`'s `notification` row grew by a full paragraph against the file's own "keep this file short and stable" header rule. Raised as *context*, not asserted — reviewer precedent is inconsistent (accepted on #436/#437, fixed on #438) | fixed-in-`1988067` — **accepted the critique**: #436's "a pattern twelve slices repeated is what a worsening violation looks like" is right, and the detail has two better homes that already hold it (the `ObservabilityMetrics` Javadoc and the runbook section). Trimmed to the load-bearing facts |
| F-4 | self (docs-accuracy, during the gate) | `CLAUDE.md`'s spine-flow paragraph enumerates the `BookingConfirmed`/`BookingCancelled` subscribers as `payout` plus `booking`'s refund listener, never `notification`. Pre-existing, but this slice adds a second `notification` listener to that fan-out, so leaving it makes the paragraph describe a spine the code no longer has | fixed-in-`1988067` — one clause, and it now says why three subscribers on `BookingCancelled` forced #405's listener-id scope |
| F-6 | review (git-history reviewer) | `booking.events.BookingCancelled`'s Javadoc asserts *"So `payout` is the sole subscriber."* — true when written, made **false** by this slice. The file is not otherwise in the diff, which is exactly why it was easy to miss: a doc the slice invalidates without touching | fixed-in-`FIX2SHA` — names `notification` as the second subscriber and says why it changes nothing above it (read-only, off the money path) |
| F-7 | review (git-history reviewer) | `RESPONSIBILITIES.md`'s numbered spine recap (step 9) still listed two consumers of `BookingCancelled` while the same file's `notification` section, edited by this slice, described the third — the file contradicting itself | fixed-in-`FIX2SHA` |
| F-8 | review (git-history reviewer, minor) | `TransactionalMailService.isSuppressedOrFailOpen`'s Javadoc said the carve-out is "deliberately not shared with `sendBookingConfirmation`" — now two registry sends share that reasoning | fixed-in-`FIX2SHA` — both named, here and in the class-level twin |
| F-9 | review (code-comment reviewer) | `ObservabilityMetrics.MAIL_RECOVERY_FAILED`'s Javadoc still said "do not sum the **four** mail counters", and `MAIL_REGISTRY_SHED`'s enumeration of "each later mail loss" omitted the new name — the slice fixed the sibling's docstring three paragraphs away and missed these | fixed-in-`FIX2SHA` |
| F-10 | review (code-comment reviewer) | `docs/runbooks/observability.md` still called the confirmation counter "the only one of the **four** that is never retried by anything" — wrong on both the count and the uniqueness, since this slice's own new section makes the identical claim about its sibling | fixed-in-`FIX2SHA` — "one of the two", with a pointer to where they differ |
| F-11 | review (code-comment reviewer) | **Self-inflicted:** `MissingBookingFact`'s Javadoc hedged "and, once #374's listener lands, `MAIL_CANCELLATION_ABANDONED`". Written in phase 1 because the constant did not exist yet, and never upgraded in phase 3 when it did — stale in the commit that shipped it | fixed-in-`FIX2SHA` — both counters now `@link`ed as settled fact |
| F-12 | review (code-comment reviewer) | `MailListenerExecutorArchitectureTest`'s class Javadoc still framed #374 as a mail "to write", while the test method this slice added below it calls #374 "the second listener the class Javadoc predicted" — the same file in two tenses | fixed-in-`FIX2SHA` — records that #374 arrived **compliant rather than caught**, which is what a fitness function is for; #373 still outstanding |
| F-13 | review (code-comment reviewer) | `notification/package-info.java` — the module's front door — described `adapter/in` as "the `BookingConfirmed` listener" and the grants as "the `BookingConfirmed` listener's reads", both singular. Not in the diff, undercounted by it | fixed-in-`FIX2SHA` — two listeners, and notes the grants did **not** widen, because both assemble through one resolver |
| F-5 | `riviera-docs-freshness` (run over `origin/main...HEAD`) | Two stale counts in `RESPONSIBILITIES.md` the slice invalidated without touching: "the first of the **four** mail counters to move in a relay outage" (line ~321) and "all **four** mail counters have a single reader today" (line ~439). Exactly the #72 failure mode this skill exists to catch — a number in a doc every session loads | fixed-in-`1988067` — both now five |
