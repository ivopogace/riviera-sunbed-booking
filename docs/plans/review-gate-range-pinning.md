...
| F-22 | history review | Both plan-recovery recipes (`CLAUDE.md`, `riviera-docs-freshness`) are `git log --all --diff-filter=D`, the command this slice's own table declares unreliable on a shallow clone — and it answers with **silence**, not an error. Both files are in the diff, so RV-PROC-2 was due on them | fixed — each recipe now names the precondition |
| F-23 | history review | `pr-gates.md`'s announcement template still read `<base-sha>...HEAD`, the surviving half of the defect F-21 fixed one line above it | fixed — both ends literal in the template too |
| F-24 | history review | The generalization-audit row claimed hits its own command does not produce (`ci.yml:219` wraps across lines) and missed `CONTRIBUTING.md:123`, which it does reach and which was unpatched | fixed — row corrected to the command's real output, `CONTRIBUTING.md` patched |
| F-25 | history review | `riviera-local-debug`'s `git merge-base` row re-blurred the split F-19 was raised to fix: the truncated-graph wrong answer and the throw-then-fail-open are two cases, and the warning fires only on the second | fixed — the row states both |
| F-21 | self, during the gate | The gate's own new instruction said to pin a dispatched agent to the resolved **base** SHA, leaving the head side a ref. I followed it literally and one review agent spent 51 minutes reading `…...HEAD` while eight fix commits moved `HEAD` under it | fixed — the instruction now pins both ends, which is the case that actually bites |
...
- `CONTRIBUTING.md` — the same over-claiming glob, found by the history review re-running the audit's own command
...
| 2026-09-05 | review round (#942) | Sentences that **count** guards or suites — the class PR #618's F-5 and this slice's own F-3 landed in | `grep -rn "check-\*\.mjs\|by-hand verifier\|spawns each guard" CLAUDE.md CONTRIBUTING.md .github/workflows .claude/skills scripts` | Re-run at the history review, the command's real output is: `CLAUDE.md:62`, `:64`, `CONTRIBUTING.md:123`, `git-diff.mjs:2`, `guard-cli-harness.mjs:2`, `guard-cli.test.mjs:2` | `CLAUDE.md` and `CONTRIBUTING.md` corrected; the three script headers are accurate as they stand. `ci.yml:219` and `README.md:64` were **also** corrected but neither came from this command — see the row below, which is the finding |
| 2026-09-05 | history review (#942) | The audit row above claimed hits the command does not produce and missed one it does — the RV-PROC-2b class ("a procedure that cannot reach its own stated scope reports zero findings, which reads as good news") | the same command, actually run | `ci.yml:219`'s "one that spawns each guard" **wraps** across two lines, so no single-line grep reaches it; `README.md` was patched though never in the searched file list; `CONTRIBUTING.md:123` was reached and not patched | Row corrected to its real output. The lesson for the next sweep: a phrase that wraps needs a shorter anchor or `-z`, and the file list must cover every file the claim is about, not the ones that came to mind |
