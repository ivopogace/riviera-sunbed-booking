...
|---|---|---|---|
| F-1 | review (`/code-review`, agent 2 + agent 3) | **False positive — rejected after verification.** Claimed V34's `E' \t\n\r\f\v'` ends in the literal letter `v` (Postgres not recognising `\v`), so the CHECK would btrim `v` and reject `vodafone.al`. Verified directly against `postgres:17`: `SELECT ascii(...)` over the set returns `32,9,10,13,12,`**`11`** — the 6th char *is* vertical tab — and `btrim('vodafone.al', …)` returns it unchanged. The reviewer tested `postgres:16-alpine`; our container is `postgres:17`, and the IT passed the whole time. **No code change**, but the canary test was kept and its rationale rewritten: it now pins the escape-set semantics rather than claiming a bug | rejected, test kept — `<review-fix>` |
| F-2 | review (agent 3 + agent 5, independently) | **Real.** `Emails.normalize` trims the whole address, never the domain substring, so `suppress("user@ x.com")` produced `domain = " x.com"` — which V34's CHECK rejects, contradicting the migration's own "accepts precisely what the writer emits" claim and raising a `DataIntegrityViolationException` on the drainer thread | fixed in `<review-fix>` — the adapter now refuses an edge-padded domain up front, so Java rejects it before the DB has to |
| F-3 | review (agent 3) | **Real.** Fail-open caught the whole `DataAccessException` hierarchy, but the decision was argued only for transient wedges; a revoked grant or schema drift would have failed open *forever*, mailing every suppressed address behind one log line | fixed in `<review-fix>` — narrowed to `TransientDataAccessException`; structural failures fall through to the outer catch and drop the mail as before. Pinned by `TransactionalMailServiceTest.aStructurallyBrokenSuppressionReadDoesNotFailOpen` |
| F-4 | review (agent 5) | **Real.** `TransactionalMailService`'s class Javadoc still claimed the invariant is "enforced here for both vehicles" with no mention of the fail-open carve-out — R-3's mitigation had landed only on the private method | fixed in `<review-fix>` |
| F-5 | review (agent 4 + agent 5) | **Real.** `CustomerPasswords`' Javadoc still advertised "+ email normalization" after this PR deleted `normalizeEmail` from it | fixed in `<review-fix>` |
| F-6 | review (agent 4) | **Real.** `docs/plans/per-username-login-throttle.md:116` still cited `CustomerPasswords.normalizeEmail` as "the one canonical email normaliser". The phase-1 generalization audit searched `platform/src` only, so it structurally could not catch a `docs/` reference — the same blind spot a reviewer flagged on PR #379 | fixed in `<review-fix>` — amended in place with an "Updated by #386" note, including the honest correction that the filter now does import one `customer::vocabulary` type |
| F-7 | review (agent 5) | **Real (overclaim).** `JdbcEmailSuppressions`' Javadoc listed "a saturated pool" among the wedges the new `queryTimeout` closes; `setQueryTimeout` bounds only execution on an already-acquired connection, so pool exhaustion is still the pool's own acquisition timeout | fixed in `<review-fix>` — claim corrected and the limit stated explicitly |
| F-8 | review (agent 1) | **Accepted deviation, not fixed.** `Emails` is a static helper in `customer::vocabulary`, a surface CLAUDE.md defines as holding "typed ids/value/outcome types". The placement was chosen deliberately with the maintainer at the intake gate after `shared` proved cyclic, and it passes `PublishedSurfacePlacementArchitectureTests` (which rejects only plain interfaces there). Recorded here rather than silently: the alternative shapes are a `NormalizedEmail` value record (a wider refactor of six call sites) or leaving six copies | open — flagged to the maintainer in the PR body; no change this slice |
