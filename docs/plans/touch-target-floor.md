...
| F-23 | re-review | `touch-target.spec.ts`'s reframed TSDoc claimed "every native control kind"; the host renders button/a/input only | fixed in `<rr-sha>` |
| F-22 | re-review | A partially clipped control was still measured at its full box, so the clipping fix closed only the fully-off-screen half | fixed in `<rr-sha>` — the helper now measures the clamped box, so a half-cut control reports the part that is actually hittable |
| F-21 | re-review | The exemption swap left `admin-audit.ts` with a dangling `TouchTarget` import and zero usages | fixed in `<rr-sha>` |
| F-20 | re-review | The my-bookings marker was the back-link, which renders in the loading skeleton too — the sweep could pass having rendered nothing | fixed in `<rr-sha>` — anchored on `browse-beaches`, which only the settled state renders |
| F-19 | re-review | F-8's fix opened the takedown confirm, which **replaces** the Remove button it was meant to measure; the resting test never selected a venue, so no slot rendered there either | fixed in `<rr-sha>` — the test now measures the occupied slot first, then opens the confirm |
| F-18 | re-review | The bulk-mode click added for F-6 moved the beach-map sweep **off** the per-set editor, leaving `app-set-editor` — the surface #600 built — covered by no sweep at all | fixed in `<rr-sha>` — both modes now have their own sweep |
| F-17 | re-review | F-10 removed the false `/booking/pay` TSDoc claim instead of adding the missing sweep, leaving that page's own chrome unmeasured | fixed in `<rr-sha>` — the page is swept, and its `.link` anchors turned out to be 21 px |
| F-16 | re-review | **The reduced-motion fix did not work.** The relocated guard sat at line 63 while the unconditional `::before` transition and the hover rule came later at equal specificity and won — F-4 was still live | fixed in `<rr-sha>` — the guard now follows the rules it overrides |
| F-15 | re-review | **`clippedAway` conflated "scrolled out of a scrollport" with "permanently clipped"**, silently dropping ~14 of 24 beach-map tiles and everything below the booking dialog's fold — the exact false negative the fix was meant to close, in the code written to close it | fixed in `<rr-sha>` by rewrite: the walk clamps only on `hidden`/`clip` ancestors and **stops at the nearest scrollable one**, since past it the position is the user's to change |
| F-14 | re-review | **A keyboard regression I introduced clearing the Sonar smells.** Dropping `tabindex="0"`/`role="group"` from the scrolling map leaves a fully-sold day — every tile an inert `<span>` — with no focusable descendant, so the map cannot be scrolled by keyboard at all | fixed in `<rr-sha>`: the tab stop is back on both grids, and `operator-daily.e2e.ts` now seeds an all-locked day and runs axe, which is the test that would have caught it |
| F-13 | review gate | `touch-target.spec.ts` asserted only that the directive's own host class names reach the DOM — the directive checked against itself | fixed in `d29c838a`: the spec now states what jsdom can settle (attachment + class merge) and points at the e2e sweep as the floor's only real proof |
| F-12 | review gate | TSDoc rewritten as changelog with an issue number, against `frontend/.claude/CLAUDE.md`'s TSDoc bar | fixed in `d29c838a` |
| F-11 | review gate | The in-sentence exemption was applied two opposite ways: `data-touch-exempt` on the auth toggle, the 44px floor on six identical admin retry controls | fixed in `d29c838a` — all six now take the exemption, since prose-plus-inline-button is the same shape in both places |
| F-10 | review gate | The tourist spec's TSDoc claimed `/booking/pay` was swept to prove the Stripe iframe is skipped; no test navigated there | fixed in `d29c838a` — the doc now says the exemption rests on the iframe boundary, not on an assertion |
| F-9 | review gate | The sweep helper's visibility filter accepted a control clipped away by an `overflow:hidden` ancestor, which still reports full geometry — the third helper blind spot of the slice, and how F-6 survived a green sweep | fixed in `d29c838a`: the helper now walks clipping ancestors and rejects a control with no visible intersection |
| F-8 | review gate | The admin photos sweep was **vacuous**: the mock returned no photos, so neither the Remove button nor the takedown confirm the test is named after ever rendered | fixed in `d29c838a` — the mock occupies a slot, the test selects a venue and opens the confirm |
| F-7 | review gate | The daily grid rendered ragged: actionable tiles grew to 44px while the locked (`BOOKED_ONLINE`) branch kept `h-8` | fixed in `d29c838a` |
| F-6 | review gate | **The layout editor's paint grid was clipped and unreachable.** It took the 44px column floor in phase 2 without the `overflow-x-auto` scroller the daily grid got, and `BeachGridFrame` is `overflow-hidden`, so a wide venue lost its far columns entirely — unpaintable | fixed in `d29c838a`; the regression test was verified **red on the pre-fix code** before being kept |
| F-5 | review gate | `/me/bookings` was on no sweep, and its two anchors were skipped, leaving a ~20px link on a shipped tourist surface | fixed in `d29c838a` — both floored in SCSS plus a new sweep for the route |
| F-4 | review gate | The reduced-motion guard still named `.riv-mobile-swatch` after phase 5 moved the transition onto its `::before`, so the opt-out stopped working for the users who asked for it | fixed in `d29c838a` |
