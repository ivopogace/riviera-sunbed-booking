...
| F-20 | re-review | The my-bookings marker was the back-link, which renders in the loading skeleton too — the sweep could pass having rendered nothing | fixed in `69eb9e7f` — anchored on `browse-beaches`, which only the settled state renders |
| F-19 | re-review | F-8's fix opened the takedown confirm, which **replaces** the Remove button it was meant to measure; the resting test never selected a venue, so no slot rendered there either | fixed in `69eb9e7f` — the test now measures the occupied slot first, then opens the confirm |
| F-18 | re-review | The bulk-mode click added for F-6 moved the beach-map sweep **off** the per-set editor, leaving `app-set-editor` — the surface #600 built — covered by no sweep at all | fixed in `69eb9e7f` — both modes now have their own sweep |
| F-17 | re-review | F-10 removed the false `/booking/pay` TSDoc claim instead of adding the missing sweep, leaving that page's own chrome unmeasured | fixed in `69eb9e7f` — the page is swept, and its `.link` anchors turned out to be 21 px |
| F-16 | re-review | **The reduced-motion fix did not work.** The relocated guard sat at line 63 while the unconditional `::before` transition and the hover rule came later at equal specificity and won — F-4 was still live | fixed in `69eb9e7f` — the guard now follows the rules it overrides |
| F-15 | re-review | **`clippedAway` conflated "scrolled out of a scrollport" with "permanently clipped"**, silently dropping ~14 of 24 beach-map tiles and everything below the booking dialog's fold — the exact false negative the fix was meant to close, in the code written to close it | fixed in `69eb9e7f` by rewrite: the walk clamps only on `hidden`/`clip` ancestors and **stops at the nearest scrollable one**, since past it the position is the user's to change |
| F-14 | re-review | **A keyboard regression I introduced clearing the Sonar smells.** Dropping `tabindex="0"`/`role="group"` from the scrolling map leaves a fully-sold day — every tile an inert `<span>` — with no focusable descendant, so the map cannot be scrolled by keyboard at all | fixed in `69eb9e7f`: the tab stop is back on both grids, and `operator-daily.e2e.ts` now seeds an all-locked day and runs axe, which is the test that would have caught it |
