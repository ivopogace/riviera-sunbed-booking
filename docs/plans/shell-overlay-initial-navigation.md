...
| F-9 | re-review (bug scan) | `markOverlayRaised()` also fired on the closing half of the three toggles, so what keeps `find-booking.ts:191` honest was emergent rather than stated | fixed — each toggle records only when it is opening |
