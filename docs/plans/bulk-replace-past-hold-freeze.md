...
`riviera-review-overlay` (review gate — **ran**: `/code-review` at **high** effort, the
no-exceptions tier for an availability-touching slice, layered with this overlay; **9 findings, all
fixed** (F-8…F-16), the dominant one a coverage hole on the very guard this slice re-scopes — no
test had ever held a set other than the venue's *first*, so a probe narrowed to `existing.getFirst()`
survived the whole suite. Re-walked over the fix round: RV-BE-1 strengthened, RV-BE-6 one violation
fixed, RV-BE-9 unchanged and clean) · `riviera-docs-freshness`
...
| R-5 | Removing `anyClaims` from a **published** `venue::spi` named interface breaks a consumer this session did not find | low | med | the port has exactly one implementor (`availability`) and, after this slice, zero callers — verified by `git grep -n "anyClaims"` over `platform/src`, which returns only the declaration, the impl, the one caller being changed, and the test fake. A missed consumer cannot compile, so this fails loudly at build time, never at runtime. `ModularityTests` + `PublishedSurfacePlacementArchitectureTests` in the scoped run | agent | closed — `git grep -n "anyClaims\b"` over `platform/src` returns nothing (the docs-freshness sweep re-verified independently); a missed consumer could only have failed **compilation**, never runtime. Structural net green (`ModularityTests`, `PackageShapeArchitectureTests`, `PublishedSurfacePlacementArchitectureTests`) |
| R-6 | Query-plan regression: the venue-wide call moves from a bare `set_id IN (…)` `EXISTS` to one carrying an extra `booking_date >= :from` | low | low | none needed — `set_availability_uniq (set_id, booking_date)` leads on `set_id` and carries `booking_date` second, so the added predicate is an index-range narrowing on the same scan, with the same IN-list size the old probe already used. `postgres` consulted; no index change | agent | closed — no index change needed: the added predicate is a range narrowing on the same `set_availability_uniq` composite's second column, same leading column and same venue-sized IN-list. Sonar reported 0 new issues and 100% new-code coverage |
| R-7 | Stale prose outlives the change: four merged plan docs, two `(any date)` Javadocs and `RESPONSIBILITIES.md` describe a guard that no longer exists, and a future reader reinstates `anyClaims` as a "regression fix" | med | low | this is exactly #599's F-2/F-1 failure mode, so the docs sweep is a **planned phase** (phase 2), not a close-out afterthought: `EditBeachMap`, `ReplaceRejection`, `SetAvailabilityLookup`, `RESPONSIBILITIES.md` §`venue` in the code phases, then `riviera-docs-freshness` over the whole range for what the diff cannot show | agent | closed — **the risk materialised and the mitigation caught it.** The phase-2 sweep patched the named docs; `riviera-docs-freshness` then found 7 more (F-1…F-7), including two frontend TSDocs and a plan doc's interface tables no review of the code diff could reach; the review gate then found 3 further stale claims in the same class (`repriceRow`'s twin javadoc, the replace IT's class javadoc, and an over-claiming port contract). All patched |
...
| 4 — Review-gate fix round (F-8…F-16) | ✅ | `<r-round>` |
...
| F-8 | review (`/code-review`, **high**) | **A coverage hole on the guard this slice re-scopes.** No test ever placed a live hold on anything but the venue's *first* set, so mutating the probe to `hasLiveHold(List.of(existing.getFirst()))` survived the entire suite — while in production a mark on any other set is CASCADE-deleted by `deleteAllSets` (invariant #2) | fixed-in-`<r-round>` — new `VenueAdminServiceTest.rejectsReplaceWhenAnyLockedSetIsHeldNotJustTheFirst` holds the **last** of two locked sets, the AC-4 ordering test now asserts `List.of(SET, later)`, and `BeachMapReplaceIT`'s live-hold IT moved its hold to `setIds(venue).getLast()`. The mutation now fails at both levels |
| F-9 | review (`/code-review`, **high**) | **The fake that let F-8 hide.** F-1's rewrite answered from a single `holdOn` date and ignored `setIds` entirely, so no test could make its verdict depend on which sets were probed — and it broke the port's documented empty-input contract (`false` without a query) | fixed-in-`<r-round>` — holds are now keyed **per set and by date** (`Map<SetId, LocalDate>`), and `anyClaimsFrom` streams the probed ids, so an empty input is `false` and the answer tracks *which* sets were asked about. This is what makes F-8's new test able to fail |
| F-10 | review (`/code-review`, **high**) | `EditBeachMap#repriceRow`'s Javadoc carried the **byte-identical** stale token-bump claim ("the bump precedes the reprice `UPDATE`, so a rejected `NO_SUCH_ROW` may still bump it") that this slice had just corrected on `replaceLayout` 20 lines below — same defect class, same file, missed | fixed-in-`<r-round>` — corrected to bump-follows-success, matching `VenueAdminService:221-225` |
| F-11 | review (`/code-review`, **high**) | My F-5 rewrite of `RESPONSIBILITIES.md` widened "a delete only sweeps a hold not yet honoured" to "a delete **or replace**", producing a paragraph that contradicted itself: it then concluded last season's cancelled booking cannot freeze the map, which is false for both `removeSet` and the replace (and by design) | fixed-in-`<r-round>` — the two reliefs are now stated separately and sized: **total** on the availability arm, **edit-only** on the booking arm, with the permanent `LAYOUT_IN_USE` named explicitly as by-design |
| F-12 | review (`/code-review`, **high**) | `BeachMapReplaceIT`'s class Javadoc still said the `set_version` bump "is acquired before the invariant-#2 lock" — the same reversed claim as F-10, in a block this slice had reworded four lines earlier | fixed-in-`<r-round>` — the version is *read* under the venue row lock before the set locks, and bumped only on success |
| F-13 | review (`/code-review`, **high**) | My rewritten `SetAvailabilityLookup#anyClaimsFrom` contract over-claimed: "the one availability question **every** layout write asks". `addSet` runs no probe and `repriceRow` is documented probe-free, so a reader could take `addSet`'s missing probe for a bug — or add one, refusing a new set on a venue that merely holds a mark elsewhere | fixed-in-`<r-round>` — scoped to every write that **disturbs** a set, with the two non-probing writes named |
| F-14 | review (`/code-review`, **high**) | `concurrentWalkInMarkAndReplaceNeverSilentlyLoseTheHold` derived its hold date from `LocalDate.now()` — the **JVM default zone**, which invariant #6 forbids — in the very file this slice had just given a `TIRANE` constant for that reason. Masked today by the +2y offset, but the guard became date-sensitive in this slice and the phase-1 audit swept only `DATE '…'` literals | fixed-in-`<r-round>` — `LocalDate.now(TIRANE)` |
| F-15 | review (`/code-review`, **high**) | My bulk comment edit mis-applied one: `editSetAppliesAPriceOnlyChangeToAClaimedSet` claimed "a hold dated today still blocks" while asserting `APPLIED` — a price-only edit never reaches the probe, so the seeded hold is inert and the comment described the opposite of the test's point | fixed-in-`<r-round>` — "live, yet inert: a price-only edit never probes" |
| F-16 | review (`/code-review`, **high**) | The risk register's Resolution column was **shifted one row** from R-5 on (my scripted close-out), so R-5/R-6/R-7 were each closed by the previous risk's evidence and R-7's real closure was lost — in the durable record later slices consult | fixed-in-`<r-round>` — each row re-paired with its own evidence |
