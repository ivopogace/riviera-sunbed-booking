...
| F-8 | review (`/code-review`, **high**) | **A coverage hole on the guard this slice re-scopes.** No test ever placed a live hold on anything but the venue's *first* set, so mutating the probe to `hasLiveHold(List.of(existing.getFirst()))` survived the entire suite — while in production a mark on any other set is CASCADE-deleted by `deleteAllSets` (invariant #2) | fixed-in-`eb82c2e` — new `VenueAdminServiceTest.rejectsReplaceWhenAnyLockedSetIsHeldNotJustTheFirst` holds the **last** of two locked sets, the AC-4 ordering test now asserts `List.of(SET, later)`, and `BeachMapReplaceIT`'s live-hold IT moved its hold to `setIds(venue).getLast()`. The mutation now fails at both levels |
| F-9 | review (`/code-review`, **high**) | **The fake that let F-8 hide.** F-1's rewrite answered from a single `holdOn` date and ignored `setIds` entirely, so no test could make its verdict depend on which sets were probed — and it broke the port's documented empty-input contract (`false` without a query) | fixed-in-`eb82c2e` — holds are now keyed **per set and by date** (`Map<SetId, LocalDate>`), and `anyClaimsFrom` streams the probed ids, so an empty input is `false` and the answer tracks *which* sets were asked about. This is what makes F-8's new test able to fail |
| F-10 | review (`/code-review`, **high**) | `EditBeachMap#repriceRow`'s Javadoc carried the **byte-identical** stale token-bump claim ("the bump precedes the reprice `UPDATE`, so a rejected `NO_SUCH_ROW` may still bump it") that this slice had just corrected on `replaceLayout` 20 lines below — same defect class, same file, missed | fixed-in-`eb82c2e` — corrected to bump-follows-success, matching `VenueAdminService:221-225` |
| F-11 | review (`/code-review`, **high**) | My F-5 rewrite of `RESPONSIBILITIES.md` widened "a delete only sweeps a hold not yet honoured" to "a delete **or replace**", producing a paragraph that contradicted itself: it then concluded last season's cancelled booking cannot freeze the map, which is false for both `removeSet` and the replace (and by design) | fixed-in-`eb82c2e` — the two reliefs are now stated separately and sized: **total** on the availability arm, **edit-only** on the booking arm, with the permanent `LAYOUT_IN_USE` named explicitly as by-design |
| F-12 | review (`/code-review`, **high**) | `BeachMapReplaceIT`'s class Javadoc still said the `set_version` bump "is acquired before the invariant-#2 lock" — the same reversed claim as F-10, in a block this slice had reworded four lines earlier | fixed-in-`eb82c2e` — the version is *read* under the venue row lock before the set locks, and bumped only on success |
| F-13 | review (`/code-review`, **high**) | My rewritten `SetAvailabilityLookup#anyClaimsFrom` contract over-claimed: "the one availability question **every** layout write asks". `addSet` runs no probe and `repriceRow` is documented probe-free, so a reader could take `addSet`'s missing probe for a bug — or add one, refusing a new set on a venue that merely holds a mark elsewhere | fixed-in-`eb82c2e` — scoped to every write that **disturbs** a set, with the two non-probing writes named |
| F-14 | review (`/code-review`, **high**) | `concurrentWalkInMarkAndReplaceNeverSilentlyLoseTheHold` derived its hold date from `LocalDate.now()` — the **JVM default zone**, which invariant #6 forbids — in the very file this slice had just given a `TIRANE` constant for that reason. Masked today by the +2y offset, but the guard became date-sensitive in this slice and the phase-1 audit swept only `DATE '…'` literals | fixed-in-`eb82c2e` — `LocalDate.now(TIRANE)` |
| F-15 | review (`/code-review`, **high**) | My bulk comment edit mis-applied one: `editSetAppliesAPriceOnlyChangeToAClaimedSet` claimed "a hold dated today still blocks" while asserting `APPLIED` — a price-only edit never reaches the probe, so the seeded hold is inert and the comment described the opposite of the test's point | fixed-in-`eb82c2e` — "live, yet inert: a price-only edit never probes" |
| F-16 | review (`/code-review`, **high**) | The risk register's Resolution column was **shifted one row** from R-5 on (my scripted close-out), so R-5/R-6/R-7 were each closed by the previous risk's evidence and R-7's real closure was lost — in the durable record later slices consult | fixed-in-`eb82c2e` — each row re-paired with its own evidence |
