...
RV-FE-E2E owns the phase-3 spec's suite placement) · `riviera-docs-freshness` (**ran** over `origin/main..HEAD`,
**1 finding, patched**: F-6 below — the rename/removal grep and the counting sweep both came back
clean, but walking the map into the sibling plan docs found that #829's review had hit this slice's
F-3 finding verbatim and that the maintainer's recorded remedy was *relocate, don't delete*. The
counting sweep's seven hits were all other subjects: "the two-suite e2e split" and "the token
registry lives in two places" both stay true, since this slice adds a spec to an existing suite and
registers no token) · `riviera-tailwind`
...
|---|---|---|---|
| F-1 | review (RV-PROC-1, self-found walking the overlay) | *Skills consulted* omitted **`playwright-cli`** and **`angular-developer`** + the angular-cli MCP, though the diff adds an e2e spec and edits nine templates. `playwright-cli` was in fact loaded before phase 3 — the line, not the routing, was wrong; `angular-developer` had not been consulted at all | fixed-in-`df0604e` — consulted both for real (not merely listed) and recorded what each confirmed |
| F-3 | review (prior-PR sweep — the same TSDoc point PR #833's review raised, id 5471842303) | `operator-error-ink.e2e.ts`'s header carried decision *history* — a parenthetical narrating that #835 closed the equivalent risk on one host and had to reopen it as F-4 — which `frontend/.claude/CLAUDE.md` asks TSDoc to avoid ("states the contract, not the changelog"). **Partially accepted:** the reviewer's premise that the house-style carve-out excludes e2e files is wrong — 18 e2e specs cite an issue number in TSDoc, both sibling token-ink specs included — so the bare `(#855)` stays. The F-4 narrative was genuine changelog, and duplicated the plan's AC-4; trimmed | fixed-in-`34a51ee`, completed in `f88211b` (see F-6) |
