...
| F-6 | review (`/code-review` fan-out, agent 3 — git history) | Risk-register row R-2 still cited `theRuleReachesTheProductionListener`, the pre-F-1 test name. `6055036` renamed the test and updated the AC and verification rows but missed this one — an internal inconsistency in the PR's own paper trail that `riviera-docs-freshness` cannot catch, since it audits substrate docs, not a plan doc's self-consistency | fixed-in-`e326a3b` |
