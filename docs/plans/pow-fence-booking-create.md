...
| F-4 | review gate (git-history agent) | `fixed-fill-state-skins.e2e.ts`'s two booking-creating tests were missed by the phase-2 sweep: the file already mentioned `mockChallengeFence` for a different test, so a file-level check read as covered. Both now route the fence off | fixed-in-`dea329b` |
