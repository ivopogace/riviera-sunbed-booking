...
- [x] **AC-9:** Given a **forced `dark` document theme**, when the same payouts net and the
      ...
      its own is the *guard spec's* mutation — AC-1 — not this one's.) **One test per site, not one
      for both:** the tabs are independent lazy route children, so the payouts net resolving under
      dark is no evidence about the daily-view notice. *Seam:* the rendered console routes under
...
| Check-in error notice renders `#a3372a` on the arrivals card glass | **preserved** | Same utility, same host (`appCardGlass` under the porcelain-pinned console). Verified under a forced dark theme by its **own** dark test, not by the payouts one — the tabs are independent lazy route children: AC-9 |
...
| F-3 | review gate (git-history agent) | **The merge-grep close-out check stated an expectation drafted before the migration and never reconciled with it.** It claimed `grep -rn 'a3372a' frontend/src/app/operator/` → "exactly two hits"; the real answer is five, because a role sweep must name the literal it excludes and `set-editor.contrast.spec.ts` carries a declined-by-design value statement — both of which phase 1's own audit log already documented. Ticked mechanically, the gate would either fail against its own stated expectation or be ticked without re-deriving the count | **fixed** — replaced with the two greps that actually discriminate (the role form → zero; the tints → present), with the five-hit explanation stated so the next reader is not surprised |
| F-4 | review gate (comment-guidance agent) | **AC-9 and the behavior-parity ledger claimed dark-theme render coverage for the daily-view check-in notice that the e2e did not provide** — the dark test opened only the payouts tab and read only `ledger-net`. The spec's own header argues the tabs are independent lazy route children, so the payouts assertion was never evidence about the notice; the plan claimed otherwise | **fixed in code, not by downgrading the claim** — the notice now has its own dark-theme test, so both sites are render-verified under a forced dark theme; AC-9 and the ledger row name both tests and say why they are two |
| F-5 | review gate (comment-guidance agent) | **`payouts-tab.contrast.spec.ts`'s docblock called the chip tint "the tab's lowest-contrast pair".** Recomputed: the chip is 5.047:1, but the weather-confirm button's white on `#9a6410` is 4.995:1 — lower. Both clear AA, so no risk; the superlative was simply wrong | **fixed** — the docblock now claims what is true and load-bearing (5.05:1 is the lowest pair *this ink* lands in anywhere, which is why the measurement lives there) and names the tab's actual lowest |
...
| 2026-08-31 | review-gate F-4 fix (new assertion added) | **Every dark-theme render assertion the slice now ships** — the mechanism a new absence-of-change test can be vacuous in, enumerated by listing the dark tests in the spec rather than assuming the new one inherits the old one's mutation result | `grep -n "dark document theme" frontend/e2e/console-negative-ink.e2e.ts` | 2 dark tests (payouts net, check-in notice) | **Both mutation-checked, not just the original.** The new check-in-notice test was run under the same combined mutation (dark override + porcelain pin flipped to `riviera`) and turned red — `Received: "rgb(255, 138, 122)"`. A new assertion added to answer a review finding gets the same scrutiny as the one it joins |
...
- [x] **Merge grep (the ticket's own check), stated as the two greps that actually discriminate.**
      ...
      value statement this slice declines to touch. The discriminating pair is:
      `grep -rn 'text-\[#a3372a\]' frontend/src/app/operator/ --include='*.ts' --include='*.html' | grep -v '\.spec\.ts'`
      → **zero** (the migration is complete), and
      `grep -n 'border-\[#a3372a\]/28\|bg-\[#a3372a\]/12' frontend/src/app/operator/payouts-tab.html`
      → **present** (#852's tints survived). Both are machine-checked by AC-4 and AC-5; this is the
      by-hand form.
