...
> its worker differently: a module worker from a same-origin script URL (`/vendor/maplibre-gl-worker.mjs`,
> set explicitly by the map adapter — it is not a Blob URL), so it needs `worker-src 'self'` beside
> Altcha's `blob:`; its style, tiles, glyphs and sprites are all same-origin (`/map/**`), so
> `connect-src 'self'` and `img-src 'self'` suffice, and it needs no third-party allowance either.
...
> nothing looks exactly like an endpoint with no traffic — so verify it in the Stripe dashboard
> whenever the endpoint is created or re-pointed.
