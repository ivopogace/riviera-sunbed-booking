...
| `riviera_refunds_adopted_total` (counter, #569) | A refund the gateway **already held** was adopted instead of created again: the adapter lists the refunds on the booking's PaymentIntent before creating one, found a live refund there, recorded it locally and reported success. An increment says an earlier attempt **moved the money but lost the response** (a read timeout), so nothing was written locally at the time — the money was always right and the record has just caught up. **Not a money-loss signal, and never summed with `riviera_refunds_failed_total`** — nothing failed here, and nothing is owed. It is a *gateway-connectivity* signal: read it beside the adapter's `WARN` lines about a timed-out refund create. One-offs are the design working. A rising rate means lost responses are routine — investigate latency to the gateway, since every one of them delays a tourist's refund record until the next replay | not on a single increment. **A sustained rate, or any increment carrying a "requested X, Stripe holds Y" WARN** — that one means the gateway holds a refund for a *different* amount than the policy computed, which the platform will not silently reconcile (the difference is `booking`'s decision, not `payment`'s). Reconcile that booking by hand |
