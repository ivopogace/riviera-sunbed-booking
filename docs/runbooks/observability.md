...
`refund.failed`/`refund.updated`/`charge.refund.updated` with no refund id or no refund **status**
(#592: the status is what the refund branch decides on, so a payload without one would otherwise read
as "still live" and be consumed) — a deliberate rollback, so the event-id dedup
