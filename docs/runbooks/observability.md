...
SET status = CASE WHEN t.refunded >= p.amount_minor THEN 'REFUNDED'
                  WHEN t.refunded > 0 THEN 'PARTIALLY_REFUNDED' ELSE 'SUCCEEDED' END,
    updated_at = NOW()
FROM (SELECT payment_id, SUM(refunded_minor) AS refunded FROM payment_booking GROUP BY payment_id) t
