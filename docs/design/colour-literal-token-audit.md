...
>   versus a porcelain-pinned subtree with no reachable dark branch) — #864's bar for sharing a name.
...
| **availability calendar `<dialog>`** (`shared/availability-calendar.html`) | `--riv-pop-*` **since #888**; before it, the pinned `--riv-calendar-*` ramp #849 registered | white `0.92`, dark ink, light opaque day tints | slate `0.96`, light ink, **dark opaque day tints** (`--riv-calendar-*`, declared in both blocks) |
