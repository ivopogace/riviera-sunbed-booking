...
| Plain `text-[#a3372a]` refund-red inks in `operator/` (`payouts-tab.ts` + `.html`, `daily-view-tab.html`) | 3 | `--riv-solid-btn-danger-ink` | That token is the outline **button**'s ink, pinned to the button's own non-theming fill (#851). These three are console inks on card glass — different role, different surface, so the same fork #848 settled: register the role's own token, don't widen the coincidental one. Found by #848's generalization sweep, which enumerated the plain ink form by mechanism; the `/opacity` tints (#852) and the button ink (#851) already had rows, the ink form had none | open → #864 |
