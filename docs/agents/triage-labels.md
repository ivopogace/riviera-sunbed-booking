...
skills + Angular MCP; `area:backend` → the `spring-modulith-*` / `riviera-stripe-payments`
skills; both always plan with `riviera-plan-doc` and review with `riviera-review-overlay`.
