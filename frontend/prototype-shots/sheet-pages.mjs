...
/**
 * PROTOTYPE — throwaway. One labelled contact sheet per surface shot by `shoot-pages.mjs`, so all
 * seven header variants can be compared in a single image instead of forty-two.
 */

const SD = process.argv[2];
const VARIANTS = (process.env.VARIANTS ?? 'current,b,c,d,e,f,g,h').split(',');
const NAMES = {
  current: 'CURRENT · shipped baseline',
  b: 'B · Pill tabs',
  c: 'C · App bar',
  d: 'D · Editorial',
  e: 'E · Floating capsule',
  f: 'F · Search-first',
  g: 'G · Side rail',
  h: 'H · Two destinations (hybrid)',
};

const img = (n) => `data:image/png;base64,${readFileSync(`${SD}/${n}`).toString('base64')}`;

const SHEETS = [
  { view: 'map', crop: 420, stack: true, width: 1280 },
  { view: 'find', crop: 900, stack: true, width: 1280 },
  { view: 'signin', crop: 560, stack: true, width: 1280 },
  { view: 'pay', crop: 620, stack: true, width: 1280 },
  { view: 'map-phone', crop: 844, stack: false, width: 390 },
  { view: 'find-phone', crop: 844, stack: false, width: 390 },
];
...
for (const { view, crop, stack, width } of SHEETS) {
  ...
  await page.screenshot({
    path: `${SD}/${process.env.SHEET ?? 'sheet'}-${view}.png`,
    fullPage: true,
  });
  await page.close();
