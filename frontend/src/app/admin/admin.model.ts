...
/**
 * A pending operator registration as the admin approval surface reports it — mirrors the
...
/**
 * A decided operator account — active or suspended — as the admin surface reports it; mirrors
...
/**
 * What an outbox lever still owes (mail or refunds); mirrors the backend
 ...
 * (one FE↔BE contract, not two). `outstanding` counts publications — never recipients or bookings: the
...
/**
 * One recorded booking-confirmation mail attempt; mirrors
...
/**
 * One booking in the mail-delivery view; mirrors
...
/**
 * One recorded admin action; mirrors the backend `AdminAuditController.AuditEntryView`.
...
/**
 * One photo slot in the admin moderation view; mirrors the backend
 * `AdminVenuePhotosResponse.SlotPhoto`. `previewUrl` is `null` exactly when the slot is empty —
 * emptiness IS the null URL, so there is no derivable boolean to keep in step.
 */
...
/**
 * A venue's photo slots as the moderation surface reads them; mirrors the backend
