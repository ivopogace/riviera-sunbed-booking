 ...
 * Both wear `--riv-card-ink` (the heading, the move list, the Done button and the receipt links) and
 * `--riv-card-ink-soft` (the saved-at line, the empty state) over a `bg-riv-console-inset/45` fill on
 * the editor's card glass, in both console themes. Values mirror the templates; an edit there must
 * re-pass here.
 */
const RECEIPT_FILL_ALPHA = 0.45;

describe.each(CONSOLE_THEMES)('RemodelReceiptPanel contrast (WCAG AA, #1034) — $name', (theme) => {
  it('the card ink meets AA (normal text) on the inset/45 — heading, moves, Done, receipt links', () => {
    expectAaOnSurfaces(theme, theme.ink, 1, (stop) => insetOver(theme, RECEIPT_FILL_ALPHA, stop));
  });

  it('the soft card ink meets AA (normal text) on the inset/45 — saved-at line, empty state', () => {
