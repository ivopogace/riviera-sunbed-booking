 ...
 * token, so that is what the sweep matches: a bare value match would fail on sites this token must
 * not touch.
 *
 * <p>Class O carries no `#0a6e85` `/opacity` selection chrome: the console's selection chrome is
 * `#2bb8d4`/`#0e8aa8`, now `--riv-select-tint`/`-edge`, and `#0a6e85` has no `/opacity` position
