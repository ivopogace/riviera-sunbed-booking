...
@Component({
  ...
  template: `<button class="inline-flex items-center gap-2">
    <app-locate-icon class="[&_svg]:size-[17px]" />
    <span>Near me</span>
  </button>`,
})
class HostSpec {}
...
describe('LocateIcon', () => {
  function render(): HTMLElement {
    const fixture = TestBed.createComponent(LocateIcon);
  ...
  it('merges its host class with the one the call site writes', () => {
    ...
    fixture.detectChanges();
    const host = (fixture.nativeElement as HTMLElement).querySelector('app-locate-icon')!;
  ...
  it('contributes no text, so the control reads as its own word alone', () => {
    ...
    expect(control.textContent?.trim()).toBe('Near me');
  });
});
