 ...
 *
 * <p>Styled in Tailwind (the go-forward) even though both consumers are still SCSS-styled: as a
 * `shared/` primitive it is a new file, not an island inside a grandfathered stylesheet. The fill is
 * a **solid** composite of the design's amber tint rather than a translucent one — the same trick
 * `.done-badge.warn` and `.failure-icon` use — so both the contrast spec and SonarCloud's static
 * analysis (`css:S7924`) compute the real ratio (5.5:1) instead of a surface-dependent guess.
 *
 ...
 *
 * <p>`.email-withheld` is retained as an inert marker class (the styling is the utilities beside it).
 */
@Component({
  selector: 'app-withheld-email-notice',
  template: `
    <p
      class="email-withheld mt-[10px] rounded-[12px] bg-[#fcf0d9] px-[12px] py-[10px] text-left text-[12px] leading-[1.45] text-[#8a5410]"
