...
describe('defaultBookingDate', () => {
  it('returns the current civil day in Europe/Tirane', () => {
    expect(defaultBookingDate(new Date('2026-06-29T12:00:00Z'))).toBe('2026-06-29');
  });

  it('matches the Tirane civil day at a month boundary', () => {
    expect(defaultBookingDate(new Date('2026-06-30T12:00:00Z'))).toBe('2026-06-30');
  });

  it('matches the Tirane civil day at a year boundary', () => {
    expect(defaultBookingDate(new Date('2026-12-31T12:00:00Z'))).toBe('2026-12-31');
  });

  it('uses the Tirane civil day, not UTC, late in the evening', () => {
    // A naive UTC reading of 23:30 UTC would wrongly give 2026-06-29, not Tirane's 06-30.
    expect(defaultBookingDate(new Date('2026-06-29T23:30:00Z'))).toBe('2026-06-30');
  });
});
