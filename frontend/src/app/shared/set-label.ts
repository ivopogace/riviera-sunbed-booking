...
/**
 * The set + tier labelling vocabulary — the one place a set's identity and its tier become
 * rendered text. Pure and side-effect free, like the sibling `availability-grid` helpers.
 *
 ...
 * ({@link touristTierLabel}). Unifying the wording is a product decision, never a refactor.
 */
...
/**
 * The set identity phrase the booking surfaces print (`Front row · Sea view · spot 1`), always
 * from the stored `rowLabel`, the one per-venue row identity. Map tile accessible names open with
 * it, so a screen reader hears what the dialog, pay page, bookings and mail print, byte for byte.
 */
