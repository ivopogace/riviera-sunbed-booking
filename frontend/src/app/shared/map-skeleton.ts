...
export const MAP_SKELETON_TILES: readonly number[] = Array.from({ length: 12 }, (_, i) => i + 1);

/**
 * The in-flight skeleton's geometry for every surface that renders a beach map, on the canvas's
 * own row contract — 4 × 12, a real venue's floor rather than the layout editor's own 4 × 6
