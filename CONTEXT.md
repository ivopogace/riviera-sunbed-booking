...
- **Pin crowd** — venues whose pins would bury each other on the Discover riviera map at the
  ...
  venues and, when they share one beach, narrows the list to it through the head's beach chip.
  Where no zoom the map offers separates them the pill inverts and its presses walk the crowd's
  venues one by one, lighting each one's row in turn. A crowd stays one real button per venue, in
  the list's order, so a keyboard walks exactly what it walks for lone pins.
  _Avoid_: cluster, marker cluster, spider.
...
- **Sheet head** — the venue sheet's one row carrying the query: the **place** (a press opens the
  **coast picker**) over the selling line (`8 of 11 selling today`, each venue's sales close for
  ...
  preview**: a pin press lights the venue's row and brings it into view, and no card opens over
  the map. _Avoid_: toolbar, filter bar.
