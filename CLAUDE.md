...
| `notification` | transactional-mail delivery (#382): the `Mailer` transports (mock vs SMTP, profile-swapped, prod-guarded), both delivery vehicles per ADR-0011 decision 5 (registry listener for ids-only payloads, bounded in-memory dispatcher for bearer-credential ones — **each on its own bounded executor, never Boot's shared `applicationTaskExecutor`**, #383: a mail send there sits on the money-path spine's pool (invariants #8/#9), so the registry listener spells out `@Async("registryMailExecutor")` + `@TransactionalEventListener` and holds no transaction across the send, rather than `@ApplicationModuleListener`, which takes no executor qualifier — pinned by `MailListenerExecutorArchitectureTest`, whose non-vacuity guard now names **all five** shipped listeners off one list (#374, then #373, which closed the set the epic named; #124's request-outcome pair extended it); the two pools differ on saturation deliberately — recovery *drops*, having no durable copy, registry *sheds*, its publication surviving for the restart republish; since #408 the registry pool's two bounds are env-tunable properties **validated at boot on both ends** — a non-positive `queue-capacity` yields a `SynchronousQueue` that sheds nearly everything, and an oversized one restores the very unbounded queue the bulkhead removed, both booting clean — and a shed increments `shared`'s `ObservabilityMetrics.MAIL_REGISTRY_SHED`, escalating **once per saturation episode**, an episode ending when the queue empties rather than when a worker picks a task up, since the latter would peg the log rate to the drain rate; a rejection during shutdown is neither counted nor escalated — **all four of those clauses are the registry pool's alone**; #415 gave the recovery pool its own counter, `MAIL_RECOVERY_DROPPED`, and it deliberately answers the same two questions the other way: **every** drop is logged, not one per episode, since a throttle only trades away lines a durable record makes redundant and this vehicle has none (ADR-0011 decision 5), and a **shutdown rejection is counted**, since with graceful shutdown unset an in-flight request can reach a closed pool and that mail is genuinely gone — a `reason` tag keeps a redeploy from reading as a degraded relay, and only saturation escalates to `ERROR`; #434 added the tag's third value, `abandoned` — the send accepted but still queued when the drain window expired, which moved no gauge at all before it — so the name reads **never ran**, not *refused* (the counting rule and the one loss it still excludes: `docs/runbooks/observability.md`); **#442 then gave all three reasons a `kind` tag** — the drop counter had named no flow, so a lost operator-approval notice (which nobody re-sends) read exactly like a lost password reset (which the user re-requests), and the runbook's "filter by `kind`" instruction matched nothing; the gap was the *seam*, not the event — `MailDispatcher.dispatch(Runnable)` never told the pool what it carried — so widening it to `dispatch(MailKind, Runnable)` closed it on the two rejection paths **and** on the drain path, where the kind has to travel into the queue and be read back past `MdcTaskDecorator`'s wrapper, since a `kind` query that silently under-counted a third of the series would beat the honest absence it replaced; `MailKind` is the one vocabulary both loss counters share, so they cannot drift into two spellings, and it names the flow rather than the person (invariant #7 keeps the address off the tag), which is what makes ADR-0011 decision 5's "mitigated only in part" clause false; #423 then counted the recovery send the dispatcher *accepts* and cannot deliver — `MAIL_RECOVERY_FAILED`, tagged by `kind` **and** by `reason`, because one swallowing catch could lose a mail to a dead relay or to a suppression lookup broken past #386's transient fail-open, and a single number for both would page toward the wrong system; it is the mail counter to read **first** in an outage, since a relay that is down fails every send while a drop needs 100 queued, and it stays at `WARN` per-loss for the mirror-image of the registry's throttle argument — the registry vehicle gets **no** twin, its transport failure propagating so `riviera.outbox.pending` already carries it, an argument that holds only for failures that *throw* — which is why #428 gave the confirmation the listener **abandons** for a missing booking/set/contact the fourth name, `MAIL_CONFIRMATION_ABANDONED`: that path returns *normally*, so the registry completes the publication and no gauge moves, making it the one mail loss `riviera.outbox.pending` cannot show; it is tagged `no-booking`/`no-set`/`no-contact` for the three modules it implicates, and escalates **per loss** to `ERROR` — unthrottled where the shed throttles and louder than the recovery vehicle's `WARN`, because neither's objection reaches it: all three facts are FK-protected and never hard-deleted (erasure tombstones in place), so it is zero in a healthy system, cannot flood, and reads as a data-integrity fault rather than a relay one; the skip semantics are unchanged — still a normal return, still a completed publication, since none of the three facts can appear later; #374's cancellation listener abandons the same three ways and takes a **fifth, sibling name**, `MAIL_CANCELLATION_ABANDONED`, not a `kind` tag on the fourth — #442 could tag because `MAIL_RECOVERY_*` names the *vehicle* and these name the *flow*; #373's payment-due listener closes the set with a **sixth**, `MAIL_PAYMENT_DUE_ABANDONED`, on the same argument (#124's two request-outcome listeners later took the fourth and fifth sibling names, `MAIL_REQUEST_DECLINED_ABANDONED` / `MAIL_REQUEST_EXPIRED_ABANDONED`), and it is the one abandoned flow whose loss is **predictive** rather than retrospective — the sweep releases the set at the mailed deadline, so the errand it opens expires, which is why its `ERROR` carries the deadline beside the ids; the `reason` dimension is shared off one enum so the flows cannot drift, and they are never summed (details: `ObservabilityMetrics`, `docs/runbooks/observability.md`); #410 then made the two pools' **worker hygiene** one decision each instead of two: a single `MdcTaskDecorator` carries the submitting request's MDC onto both, **composed** onto the registry pool via `CompositeTaskDecorator` because the shed policy already owns that slot and replacing it would silently strand the episode flag open — the *rejection* lines never needed it, `reject(...)` running on the committing thread, so what it fixes is every **worker**-thread line (the abandoned-confirmation `ERROR`, the suppression `WARN`, #370's transport failures), which invariant #7 leaves nothing else to identify; **#455 then moved that class out of this module to `shared`** and composed it onto #404's refund pool as well, so the decorator is no longer this module's to own — three pools now, one mechanism, and `WorkerContextArchitectureTest` fails the build for a fourth that omits it, which is the guard whose absence let #404 ship undecorated in the first place; and the shutdown drain is now **derived** from the relay's socket budget — one env-tunable, boot-validated `riviera.notification.mail.socket-timeout-ms` that all three `spring.mail.properties.mail.smtp.*` timeouts interpolate under both real-transport profiles, replacing a flat 5s that was *smaller* than the budget one degraded send may legitimately occupy, so a redeploy stopped waiting on live work and closed Hikari underneath it; its ceiling **is** this module's per-pool drain claim, since a drain outlasting the platform's SIGTERM grace gets the process killed mid-close — and **#456 moved the arithmetic out of this module**, because the version living here divided a *mail* budget by a *mail* pool count and so could not see #404's refund pool, while the test guarding it asserted `(a/b)*b <= a` and could never fail: `shared`'s `ShutdownBudget` now states the grace and every pool's claim, and `ShutdownDrainArchitectureTest` **discovers** draining pools from bytecode (not from the context, which would miss the `defaultCandidate = false` bulkheads and the non-bean recovery pool) and sums the claims; and when the window expires both pools **give up rather than `shutdownNow()`** — an interrupt cannot tell a send that already reached the relay from one that has not, and interrupting the first is how at-least-once becomes a duplicate), the `BookingConfirmed` confirmation mail, the **`BookingCancelled` cancellation/refund record** (#374 — the second registry-borne booking mail, sharing the confirmation's three-port assembly via one module-internal resolver; **one listener covers every cancellation channel**, subscribing to the fact rather than to `CancelBookingService`/`WeatherRefundService`, and it *renders* the server-computed refund (invariant #10) — never re-decides it — reporting a **decision, not a settlement**, since the money moves afterwards on `booking`'s own listener and can still fail), the **`BookingPaymentDue` payment-due notice** (#373 — the third registry-borne booking mail, on the same three-port assembly, and the only one that asks for an action inside a window rather than recording something settled; **whether it is warranted is decided upstream**: `booking` publishes the fact only on the accept branch where money is genuinely outstanding, because the accept transaction commits *before* `CheckoutPort.pay` answers and the other two outcomes owe nothing — the default-profile stub confirms synchronously having collected nothing, and a failed PaymentIntent reverts to `PENDING_REQUEST` — so a status read in the listener would race the very transition that makes the answer differ; its link is the code-gated `/booking/<code>` view, built at send time from the code read through `booking::api`, never from the payload, which the registry would persist in cleartext), the **`BookingRequestDeclined` / `BookingRequestExpired` request-outcome records** (#124 — the fourth and fifth registry-borne booking mails, on the same three-port assembly and the same send-time status link; plain-record copy by product decision, no CTA; warranted-upstream like #373, but published **inside** `RequestReleaseService`'s winning legs because the guarded transition settles the outcome right there — and the withdraw leg deliberately mails nothing, #123), the **operator-approval notice** (#375 — the in-memory vehicle's first *non-recovery* kind, `kind="operator-approved"`: its link is the public sign-in URL, no bearer credential, so it is the one case where decision 5's payload test does not settle the vehicle and the *trigger* does — an admin request, not a domain fact, and an event minted to carry that back to the edge that issued it would be ceremony that also persists the payload; consequence for reading the two `MAIL_RECOVERY_*` series: **"recovery" names the vehicle, the `kind` tag names the flow**, and the names stay because renaming a shipped metric breaks whatever reads it), the **booking-confirmation delivery log** (V36, #380 — one attempt row per send with its trigger and outcome, behind an ADMIN lookup-by-address + one-click resend at `/api/admin/mail-deliveries`; it exists because `completion_date` proves only that the listener *returned*, equally true of a suppression skip and a #428 abandonment, so the registry would report the two silent losses as "dispatched" — the resend publishes nothing and sends synchronously, so it re-drives no other `BookingConfirmed` consumer and the admin gets the real outcome, not "queued"), and the **email-suppression list** (V32; hashed/non-PII since V33 — peppered-HMAC key + cleartext domain, survives erasure, ADR-0012; V34 tightened the `domain` CHECK to mirror the Java writer; V35/#391 added the one sanctioned exception to never-deleted — an ADMIN-gated reinstatement that *flags* the row `reinstated_at` rather than deleting it, so `isSuppressed` reads `email_key = ? AND reinstated_at IS NULL` and a later bounce re-suppresses via the ordinary upsert) — *no send to a suppressed address*, enforced on both vehicles at the send chokepoint, with **one carve-out** (#386): the recovery vehicle fails **open** on a *transient* lookup failure (the registry vehicle still propagates, so at-least-once retries), and the lookup is bounded by an adapter-scoped `queryTimeout` — never the global property, which would also bound `availability`'s `INSERT … ON CONFLICT` claim, whose loser waits on the winner's index tuple lock (invariant #2); since **#405** an ADMIN-gated `GET`/`POST /api/admin/mail-outbox` re-drives what the registry still owes *this module*, so a failed confirmation's retry horizon is no longer "the next deploy" — scoped by **listener-id prefix, never event type**, because `BookingConfirmed` fans out to `payout`'s accrual too (`MailOutboxScopeIT`) — a claim #374 was the first to *verify* rather than trust, and it mattered: `BookingCancelled` carries the Stripe refund (#8) and the payout reversal (#9) under the same event; the framework details it corrects in #405's own text are in `RESPONSIBILITIES.md`; publishes only `notification::api`, now two role-split ports (`MailSender`, fire-and-forget; and since #400 `MailDeliverability`, the synchronous "would this be withheld" read whose one consumer is the authenticated verification-resend — the anonymous recovery flows must not reflect the answer, D-8), consumed by the root alone; since #390 it also **implements** `booking.spi.ConfirmationMailDelivery` (the inverted edge — `notification → booking` still, so nothing depends on `notification`), letting a confirmed booking's read model say the confirmation mail was withheld — `booking` gates the disclosure, this module only answers | (none — owns `email_suppression` + `booking_confirmation_mail_attempt` state, no aggregate yet) |
