...
|---|---|---|
| `venue` | venue profiles, the beach map / layout, set positions, online-vs-walk-in pool assignment, pricing, booking mode (Instant / Request), amenities + distance-to-water, venue photos (#142, ADR-0008) incl. the platform-admin takedown (`DELETE /api/admin/venues/{venueId}/photos/{slot}`, #504 — the module's one **ownership-free** photo write, on its own `VenuePhotoModeration` port so the venue-scoped `VenuePhotos` contract stays uniformly `assertOwns`-first; the moderation stance it enforces is **ADR-0013**: report-and-remove, no pre-publication review, with the audit trail it requires tracked as #507 and the CDN-purge it will require as #508), the signed-in operator's own-venues read (`GET /api/venues/mine`, S9 #277), the owner's per-set daily availability read (`GET /api/venues/{venueId}/availability`, #207 — states via `venue.spi.SetAvailabilityLookup#statesOn`, implemented by `availability`; the tourist map stays state-agnostic) | `Venue`, `BeachMap` |
