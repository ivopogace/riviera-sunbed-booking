...
| `notification` | transactional-mail delivery (#382): the `Mailer` transports (mock vs SMTP, profile-swapped, prod-guarded); both delivery vehicles per ADR-0011 decision 5 — the Event Publication Registry listener for ids-only payloads, the bounded in-memory dispatcher for bearer-credential ones — **each on its own bounded executor, never Boot's shared `applicationTaskExecutor`** (#383), with the eight `MAIL_*` loss counters and their `kind`/`reason` tags accounting every loss mode (details: `ObservabilityMetrics`, `docs/runbooks/observability.md`); the five **registry-borne booking mails** — the `BookingConfirmed` confirmation mail, the `BookingCancelled` cancellation/refund record (#374 — one listener for every cancellation channel, *rendering* the server-computed refund per invariant #10, never re-deciding it), the `BookingPaymentDue` payment-due notice (#373 — warranted upstream: `booking` publishes the fact only on the accept branch where money is genuinely outstanding), and the `BookingRequestDeclined` / `BookingRequestExpired` request-outcome records (#124 — published inside `RequestReleaseService`'s winning legs; the withdraw leg deliberately mails nothing, #123); the **operator-approval notice** (#375 — in-memory vehicle, `kind="operator-approved"`: "recovery" in `MAIL_RECOVERY_*` names the *vehicle*, the `kind` tag names the *flow*); the **booking-confirmation delivery log** (V36, #380 — one attempt row per send, behind an ADMIN lookup-by-address + one-click resend at `/api/admin/mail-deliveries`; the resend publishes nothing, so it re-drives no other `BookingConfirmed` consumer); the **email-suppression list** (V32; hashed/non-PII since V33, ADR-0012; ADMIN reinstatement-by-flag V35/#391) — *no send to a suppressed address*, enforced on both vehicles at the send chokepoint, failing **open** only on the recovery vehicle's *transient* lookup failure (#386); and the ADMIN `GET`/`POST /api/admin/mail-outbox` re-drive (#405) — scoped by **listener-id prefix, never event type**, because `BookingConfirmed` fans out to `payout`'s accrual too (`MailOutboxScopeIT`). Publishes only `notification::api` — two role-split ports: `MailSender` (fire-and-forget) and, since #400, `MailDeliverability` (the synchronous "would this be withheld" read; sole consumer the authenticated verification-resend — anonymous recovery flows must not reflect the answer, D-8) — consumed by the root alone; since #390 it also **implements** `booking.spi.ConfirmationMailDelivery` (the inverted edge — `notification → booking` still, so nothing depends on `notification`). Pool bounds, drain budget, counter semantics and per-issue history: `RESPONSIBILITIES.md` §`notification` | (none — owns `email_suppression` + `booking_confirmation_mail_attempt` state, no aggregate yet) |
