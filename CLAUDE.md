...
|---|---|---|
| `venue` | venue profiles, the beach map / layout, set positions, online-vs-walk-in pool assignment, pricing, booking mode (Instant / Request), amenities + distance-to-water, venue photos (#142, ADR-0008), the signed-in operator's own-venues read (`GET /api/venues/mine`, S9 #277), the owner's per-set daily availability read (`GET /api/venues/{venueId}/availability`, #207 — states via `venue.spi.SetAvailabilityLookup#statesOn`, implemented by `availability`; the tourist map stays state-agnostic) | `Venue`, `BeachMap` |
