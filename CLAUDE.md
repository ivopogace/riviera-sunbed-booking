...
| `booking` | bookings, booking codes, lifecycle (pending-request/awaiting-payment/confirmed/cancelled/completed/no-show/declined/expired), request accept/decline + expiry sweep (#98), cancellation-policy enforcement, the retention-basis fact for `customer`'s sweep (`customer.spi.GuestBookingHistory`, #101 Slice 2), the withheld-confirmation-mail flag on a **confirmed** booking's read model (`booking.spi.ConfirmationMailDelivery`, implemented by `notification`, #390 — never asked before payment, or the code-gated view becomes a suppression oracle) | `Booking` |
